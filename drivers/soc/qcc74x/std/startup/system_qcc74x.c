...
void SystemInit(void)
{
    ...
    *(volatile uint32_t *)(0x2000e504) = 0;
    ...
    if (em_size == 0) {
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, GLB_EM_SEL, 0x00); // GLB_WRAM160KB_EM0KB
    } else if (em_size == 32 * 1024) {
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, GLB_EM_SEL, 0x03); // GLB_WRAM128KB_EM32KB
    } else if (em_size == 64 * 1024) {
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, GLB_EM_SEL, 0x0f); // GLB_WRAM96KB_EM64KB
    } else {
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, GLB_EM_SEL, 0x00); // GLB_WRAM160KB_EM0KB
    }
...
static int ATTR_TCM_SECTION __attribute__((noinline)) system_setup_xtal_config()
{
    ...
    /* for osc */
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_AMP_CTRL_AON, 3);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_EXT_SEL_AON, 1);
#else
    /* for crystal */
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_AMP_CTRL_AON, 2);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_EXT_SEL_AON, 0);
#endif
