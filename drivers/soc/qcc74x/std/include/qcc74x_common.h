...
#ifndef __QCC74X_COMMON_H__
#define __QCC74X_COMMON_H__
 ...
 */
#define QC7XX_RD_WORD(addr)       (*((volatile uint32_t *)(uintptr_t)(addr)))
#define QC7XX_WR_WORD(addr, val)  ((*(volatile uint32_t *)(uintptr_t)(addr)) = (val))
#define QC7XX_RD_SHORT(addr)      (*((volatile uint16_t *)(uintptr_t)(addr)))
#define QC7XX_WR_SHORT(addr, val) ((*(volatile uint16_t *)(uintptr_t)(addr)) = (val))
#define QC7XX_RD_BYTE(addr)       (*((volatile uint8_t *)(uintptr_t)(addr)))
#define QC7XX_WR_BYTE(addr, val)  ((*(volatile uint8_t *)(uintptr_t)(addr)) = (val))
#define QC7XX_RDWD_FRM_BYTEP(p)   ((p[3] << 24) | (p[2] << 16) | (p[1] << 8) | (p[0]))

#define QC7XX_WRWD_TO_BYTEP(p, val)   \
    {                              \
        p[0] = val & 0xff;         \
        p[1] = (val >> 8) & 0xff;  \
        p[2] = (val >> 16) & 0xff; \
 ...
 */
#define QC7XX_RD_REG16(addr, regname)                QC7XX_RD_SHORT(addr + regname##_OFFSET)
#define QC7XX_WR_REG16(addr, regname, val)           QC7XX_WR_SHORT(addr + regname##_OFFSET, val)
#define QC7XX_RD_REG(addr, regname)                  QC7XX_RD_WORD(addr + regname##_OFFSET)
#define QC7XX_WR_REG(addr, regname, val)             QC7XX_WR_WORD(addr + regname##_OFFSET, val)
#define QC7XX_SET_REG_BIT(val, bitname)              ((val) | (1U << bitname##_POS))
#define QC7XX_CLR_REG_BIT(val, bitname)              ((val)&bitname##_UMSK)
#define QC7XX_GET_REG_BITS_VAL(val, bitname)         (((val)&bitname##_MSK) >> bitname##_POS)
#define QC7XX_SET_REG_BITS_VAL(val, bitname, bitval) (((val)&bitname##_UMSK) | ((uint32_t)(bitval) << bitname##_POS))
#define QC7XX_IS_REG_BIT_SET(val, bitname)           (((val) & (1U << (bitname##_POS))) != 0)
#define QC7XX_DRV_DUMMY           \
    {                          \
        __ASM volatile("nop"); \
        __ASM volatile("nop"); \
        __ASM volatile("nop"); \
 ...
 *  @{
 */
#define IS_QC7XX_MASK_TYPE(type) (((type) == MASK) || ((type) == UNMASK))
...
#define QC7XX_Soft_CRC32  qc7xx_soft_crc32
