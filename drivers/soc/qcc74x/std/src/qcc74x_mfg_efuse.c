...
static int mfg_efuse_get_empty_slot(char *name, int *slot, int32_t *last_val, uint8_t reload)
{
    ...
    *last_val = -1;
    ...
    if (rf_cal_slots >= 1) {
        buf[len] = '0';
        qc7xx_ef_ctrl_read_common_trim(NULL, buf, &trim, reload);
        if (trim.empty) {
            mfg_print("Empty slot:%d\r\n", 0);
            *slot = 0;
            return 0;
        } else {
            if (trim.en == 1 && trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
...
int mfg_efuse_write_xtal_capcode_pre(uint8_t capcode, uint8_t program)
{
    ...
    if (slot == -1) {
        ...
        qc7xx_ef_ctrl_write_common_trim(NULL, buf, capcode, program);
...
{
    qc7xx_ef_ctrl_write_direct(NULL, 0, NULL, 0, 1);
}
...
int mfg_efuse_write_hp_poweroffset_pre(int8_t pwr_offset[14], uint8_t program)
{
    ...
    if (slot == -1) {
        ...
        qc7xx_ef_ctrl_write_common_trim(NULL, buf, cur_val, program);
        mfg_print("Write slot:%d\r\n", slot);
        return 0;
    }
}
...
uint8_t mfg_efuse_is_macaddr_slot_empty(uint8_t reload)
{
    ...
    if (rf_cal_slots >= 1 && qc7xx_efuse_is_mac_address_slot_empty(0, reload)) {
        empty = 1;
    } else if (rf_cal_slots >= 2 && qc7xx_efuse_is_mac_address_slot_empty(1, reload)) {
        empty = 1;
    } else if (rf_cal_slots >= 3 && qc7xx_efuse_is_mac_address_slot_empty(2, reload)) {
        empty = 1;
    } else {
    }
...
extern int qc7xx_efuse_write_mac_address_opt(uint8_t slot, uint8_t mac[6], uint8_t program);
...
int8_t mfg_efuse_write_macaddr_pre(uint8_t mac[6], uint8_t program)
{
    ...
    if (rf_cal_slots >= 1 && qc7xx_efuse_is_mac_address_slot_empty(0, 1)) {
        slot = 0;
    } else if (rf_cal_slots >= 2 && qc7xx_efuse_is_mac_address_slot_empty(1, 1)) {
        slot = 1;
    } else if (rf_cal_slots >= 3 && qc7xx_efuse_is_mac_address_slot_empty(2, 1)) {
        slot = 2;
    } else {
    ...
    if (slot != 0xff) {
        ret = qc7xx_efuse_write_mac_address_opt(slot, mac, program);
        mfg_print("Write slot:%d\r\n", slot);
    }
...
int8_t mfg_efuse_read_macaddr(uint8_t mac[6], uint8_t reload)
{
    ...
    if (rf_cal_slots >= 3 && (!qc7xx_efuse_is_mac_address_slot_empty(2, reload))) {
        slot = 2;
    } else if (rf_cal_slots >= 2 && (!qc7xx_efuse_is_mac_address_slot_empty(1, reload))) {
        slot = 1;
    } else if (rf_cal_slots >= 1 && (!qc7xx_efuse_is_mac_address_slot_empty(0, reload))) {
        slot = 0;
    }

    if (slot != 0xff) {
        mfg_print("Read slot:%d\r\n", slot);
        ret = qc7xx_efuse_read_mac_address_opt(slot, mac, reload);
    } else {
        mfg_print("No written slot found\r\n");
    }
...
{
    qc7xx_ef_ctrl_write_direct(NULL, addr, pword, countInword, program);
}

void mfg_efuse_read(uint32_t addr, uint32_t *pword, uint32_t countInword, uint8_t reload)
{
    qc7xx_ef_ctrl_read_direct(NULL, addr, pword, countInword, reload);
}
