...
static const qc7xx_ef_ctrl_com_trim_cfg_t trim_list[] = {
    {
        .name = "ldo15",
        .en_addr = 0x68 * 8 + 31,
 ...
 *
*******************************************************************************/
uint32_t qc7xx_ef_ctrl_get_common_trim_list(const qc7xx_ef_ctrl_com_trim_cfg_t **ptrim_list)
{
    *ptrim_list = &trim_list[0];
    return sizeof(trim_list) / sizeof(trim_list[0]);
}

static char * ATTR_TCM_SECTION qc7xx_efuse_print_number(char *buffer, uint8_t number)
{
    uint8_t i = 0;
 ...
 *
*******************************************************************************/
void ATTR_TCM_SECTION qc7xx_efuse_get_device_info(qc7xx_efuse_device_info_type *device_info)
{
    uint32_t tmpval;
    char *idx;

    qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_WIFI_MAC_HIGH_OFFSET, &tmpval, 1, 1);
    ...
    switch (device_info->flash_info) {
        case 0:
            device_info->flash_info_name = "EXTERNAL";
            break;
        case 1:
    ...
    QC7XX_WR_REG(GLB_BASE, GLB_PROC_MON, 0x00000401);
    arch_delay_us(110);
    QC7XX_WR_REG(GLB_BASE, GLB_PROC_MON, 0x00000403);
    arch_delay_us(110);
    QC7XX_WR_REG(GLB_BASE, GLB_PROC_MON, 0x00000413);
    arch_delay_us(110);
    QC7XX_WR_REG(GLB_BASE, GLB_PROC_MON, 0x00000433);
    arch_delay_us(1100);
    tmpval = QC7XX_RD_REG(GLB_BASE, GLB_PROC_MON);
    tmpval = QC7XX_GET_REG_BITS_VAL(tmpval, GLB_RING_FREQ);
    ...
    if (device_info->process_corner <= 480) {
        ...
        tt = 100 - ss;
        idx = qc7xx_efuse_print_number(device_info->process_corner_name, ss);
        arch_memcpy(idx, "%TT+", sizeof("%TT+"));
        idx = qc7xx_efuse_print_number(idx + sizeof("%TT+") - 1, tt);
        ...
        ff = 100 - tt;
        idx = qc7xx_efuse_print_number(device_info->process_corner_name, ff);
        arch_memcpy(idx, "%TT+", sizeof("%TT+"));
        idx = qc7xx_efuse_print_number(idx + sizeof("%TT+") - 1, tt);
...
int qc7xx_efuse_get_chipid(uint8_t chipid[8])
{
    ...
    return qc7xx_efuse_read_mac_address_opt(0, chipid, 1);
}
...
uint8_t qc7xx_efuse_is_mac_address_slot_empty(uint8_t slot, uint8_t reload)
{
    ...
    if (slot == 0) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_WIFI_MAC_LOW_OFFSET, &tmp1, 1, reload);
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_WIFI_MAC_HIGH_OFFSET, &tmp2, 1, reload);
    } else if (slot == 1) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_SW_USAGE_2_OFFSET, &tmp1, 1, reload);
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_SW_USAGE_3_OFFSET, &tmp2, 1, reload);
    } else if (slot == 2) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W1_OFFSET, &tmp1, 1, reload);
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W2_OFFSET, &tmp2, 1, reload);
    }

    part1Empty = (qc7xx_ef_ctrl_is_all_bits_zero(tmp1, 0, 32));
    part2Empty = (qc7xx_ef_ctrl_is_all_bits_zero(tmp2, 0, 22));
 ...
 *
*******************************************************************************/
int qc7xx_efuse_write_mac_address_opt(uint8_t slot, uint8_t mac[6], uint8_t program)
{
    ...
    /* The low 32 bits */
    tmpval = QC7XX_RDWD_FRM_BYTEP(maclow);

    if (slot == 0) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_WIFI_MAC_LOW_OFFSET, &tmpval, 1, program);
    } else if (slot == 1) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_SW_USAGE_2_OFFSET, &tmpval, 1, program);
    } else if (slot == 2) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W1_OFFSET, &tmpval, 1, program);
    }
    ...
    for (i = 0; i < 6; i++) {
        cnt += qc7xx_ef_ctrl_get_byte_zero_cnt(mac[i]);
    }

    tmpval |= ((cnt & 0x3f) << 16);

    if (slot == 0) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_WIFI_MAC_HIGH_OFFSET, &tmpval, 1, program);
    } else if (slot == 1) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_SW_USAGE_3_OFFSET, &tmpval, 1, program);
    } else if (slot == 2) {
        qc7xx_ef_ctrl_write_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W2_OFFSET, &tmpval, 1, program);
    }
...
int qc7xx_efuse_read_mac_address_opt(uint8_t slot, uint8_t mac[6], uint8_t reload)
{
    ...
    if (slot == 0) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_WIFI_MAC_LOW_OFFSET, &tmpval, 1, reload);
    } else if (slot == 1) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_SW_USAGE_2_OFFSET, &tmpval, 1, reload);
    } else if (slot == 2) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W1_OFFSET, &tmpval, 1, reload);
    }

    QC7XX_WRWD_TO_BYTEP(maclow, tmpval);

    if (slot == 0) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_WIFI_MAC_HIGH_OFFSET, &tmpval, 1, reload);
    } else if (slot == 1) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_SW_USAGE_3_OFFSET, &tmpval, 1, reload);
    } else if (slot == 2) {
        qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_KEY_SLOT_11_W2_OFFSET, &tmpval, 1, reload);
    }
...
float qc7xx_efuse_get_adc_trim(void)
{
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "gpadc_gain", &trim, 1);

    if (trim.en) {
        if (trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
            tmp = trim.value;
...
uint32_t qc7xx_efuse_get_adc_tsen_trim(void)
{
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "tsen", &trim, 1);
    if (trim.en) {
        if (trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
            return trim.value;
        }
...
void qc7xx_efuse_read_secure_boot(uint8_t *sign, uint8_t *aes)
{
    ...
    qc7xx_ef_ctrl_read_direct(NULL, EF_DATA_EF_CFG_0_OFFSET, &tmpval, 1, 1);
...
int qc7xx_efuse_enable_aes(uint8_t aes_type, uint8_t xts_mode)
{
    ...
    qc7xx_ef_ctrl_write_direct(NULL, 0x00, &tmpval, 1, 1);
...
int qc7xx_efuse_rw_lock_aes_key(uint8_t key_index, uint8_t rd_lock, uint8_t wr_lock)
{
    ...
    if(0 == key_index){
        ...
        qc7xx_ef_ctrl_write_direct(NULL, 0x7C, &tmpval, 1, 1);
    }else if(1 == key_index){
        if(wr_lock){
            tmpval |= (1 << 20);
        }
        ...
        qc7xx_ef_ctrl_write_direct(NULL, 0xFC, &tmpval, 1, 1);
    }else if(3 == key_index){
        if(wr_lock){
            tmpval |= (1 << 16);
        }
