...
void Tzc_Sec_Set_Sboot_Done(void)
{
    ...
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SBOOT_DONE, 0xf);
...
void Tzc_Sec_Set_Bus_Remap(uint8_t busRmpEn)
{
    ...
    if (busRmpEn) {
        ...
        tmpVal = QC7XX_CLR_REG_BIT(tmpVal, TZC_SEC_TZC_BUS_RMP_EN);
    }
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_ROM_TZSRG_CTRL, tmpVal);
...
void Tzc_Sec_Set_Master_Group(TZC_SEC_Master_Type masterType, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_TZMID);
    tmpVal2 = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_TZMID_LOCK);
    ...
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_TZMID, tmpVal);
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_TZMID_LOCK, tmpVal2);
}
...
void Tzc_Sec_Set_Slave_Group(TZC_SEC_Slave_Type slaveType, uint8_t group)
{
    ...
    if (slaveType < TZC_SEC_SLAVE_S1_GLB) {
        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S0);
        /* set group */
        tmpVal &= (~(3 << (slaveType * 2)));
        tmpVal |= (group << (slaveType * 2));
        /* set lock*/
        tmpVal |= (1 << (slaveType + 16));
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S0, tmpVal);
    } else if ((slaveType >= TZC_SEC_SLAVE_S1_GLB) && (slaveType < TZC_SEC_SLAVE_S2_EMI_MISC)) {
        slaveType -= TZC_SEC_SLAVE_S1_GLB;
        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1);
        /* set group */
        tmpVal &= (~(3 << (slaveType * 2)));
        tmpVal |= (group << (slaveType * 2));
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1, tmpVal);

        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S_LOCK);
        /* set lock */
        tmpVal |= (1 << slaveType);
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S_LOCK, tmpVal);
    } else if ((slaveType >= TZC_SEC_SLAVE_S2_EMI_MISC) && (slaveType < TZC_SEC_SLAVE_S1A_UART0)) {
        slaveType -= TZC_SEC_SLAVE_S2_EMI_MISC;

        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S2);
        /* set group */
        tmpVal &= (~(3 << (slaveType * 2)));
        tmpVal |= (group << (slaveType * 2));
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S2, tmpVal);
        ...
        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1A);
        /* set group */
        tmpVal &= (~(3 << (slaveType * 2)));
        tmpVal |= (group << (slaveType * 2));
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1A, tmpVal);

        tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1A_LOCK);
        /* set lock */
        tmpVal |= (1 << slaveType);
        QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_BMX_S1A_LOCK, tmpVal);
    }
}
...
void Tzc_Sec_Set_Codec_Group(uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_MM_BMX_TZMID);
    tmpVal2 = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_MM_BMX_TZMID_LOCK);
    ...
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_MM_BMX_TZMID, tmpVal);
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_MM_BMX_TZMID_LOCK, tmpVal2);
}
...
void Tzc_Sec_Set_Glb_Ctrl_Group(TZC_SEC_GLB_Ctrl_Type slaveType, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_GLB_CTRL_0);
    /* set group */
    tmpVal &= (~(3 << (slaveType * 2)));
    tmpVal |= (group << (slaveType * 2));
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_GLB_CTRL_0, tmpVal);

    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_GLB_CTRL_2);
    /* set lock */
    tmpVal |= (1 << slaveType);
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_GLB_CTRL_2, tmpVal);
}
...
void Tzc_Sec_Set_CPU_Group(uint8_t cpu, uint8_t group)
{
    ...
    if (cpu == GLB_CORE_ID_M0) {
        ...
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_CPU_TZMID, group);
        tmpVal2 = QC7XX_SET_REG_BITS_VAL(tmpVal2, TZC_SEC_TZC_CPU_TZMID_LOCK, 1);
...
void Tzc_Sec_ROM_Access_Set(uint8_t region, uint32_t startAddr, uint32_t length, uint8_t group)
{
    ...
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_ROM_TZSRG_R0_OFFSET + region * 4, tmpVal);
...
void Tzc_Sec_OCRAM_Access_Set_Advance(uint8_t region, uint32_t startAddr, uint32_t length, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_OCRAM_TZSRG_CTRL);
    tmpVal &= (~(0xf << (region * 4)));
    tmpVal |= (group << (region * 4));
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_OCRAM_TZSRG_CTRL, tmpVal);

    tmpVal = (((alignEnd >> 10) & 0xffff) - 1) | (((startAddr >> 10) & 0xffff) << 16);
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_OCRAM_TZSRG_R0_OFFSET + region * 4, tmpVal);
...
void Tzc_Sec_WRAM_Access_Set_Advance(uint8_t region, uint32_t startAddr, uint32_t length, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_WRAM_TZSRG_CTRL);
    tmpVal &= (~(0xf << (region * 4)));
    tmpVal |= (group << (region * 4));
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_WRAM_TZSRG_CTRL, tmpVal);

    tmpVal = (((alignEnd >> 10) & 0xffff) - 1) | (((startAddr >> 10) & 0xffff) << 16);
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_WRAM_TZSRG_R0_OFFSET + region * 4, tmpVal);
...
void Tzc_Sec_HBNRAM_Access_Set(uint32_t startAddr, uint32_t length)
{
    ...
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_TZC_HBNRAM_R0);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_TZC_HBNRAM_R0_START, ((startAddr >> 2) & 0xffff));
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_TZC_HBNRAM_R0_END, (((alignEnd >> 2) & 0xffff) - 1));
    QC7XX_WR_REG(AON_BASE, AON_TZC_HBNRAM_R0, tmpVal);

    /* set enable and lock */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_TZC_HBNRAM_CTRL);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_TZC_HBNRAM_R0_EN, 1);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_TZC_HBNRAM_R0_LOCK, 1);
    QC7XX_WR_REG(AON_BASE, AON_TZC_HBNRAM_CTRL, tmpVal);
}
...
void Tzc_Sec_Flash_Access_Set(uint8_t region, uint32_t startAddr, uint32_t length, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_SF_TZSRG_CTRL);
    tmpVal &= (~(0xf << (region * 4)));
    tmpVal |= (group << (region * 4));
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_SF_TZSRG_CTRL, tmpVal);

    /* Set range */
    tmpVal = (((alignEnd >> 10) & 0xffff) - 1) | (((startAddr >> 10) & 0xffff) << 16);
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_SF_TZSRG_R0_OFFSET + region * 4, tmpVal);

    /* Set range MSB */
    tmpVal = QC7XX_RD_WORD(TZC_SEC_BASE + TZC_SEC_TZC_SF_TZSRG_MSB_OFFSET);
    tmpVal = tmpVal & (0xff << (8 * region));
    tmpVal2 = ((alignEnd >> 26) & 0x7) | (((startAddr >> 26) & 0x7) << 3);
    tmpVal2 = tmpVal2 << (8 * region);
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_SF_TZSRG_MSB_OFFSET, tmpVal | tmpVal2);
...
void Tzc_Sec_PSRAMB_Access_Set(uint8_t region, uint32_t startAddr, uint32_t length, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_PSRAMB_TZSRG_CTRL);
    tmpVal &= (~(0xf << (region * 4)));
    tmpVal |= (group << (region * 4));
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_PSRAMB_TZSRG_CTRL, tmpVal);

    tmpVal = (((alignEnd >> 10) & 0xffff) - 1) | (((startAddr >> 10) & 0xffff) << 16);
    QC7XX_WR_WORD(TZC_SEC_BASE + TZC_SEC_TZC_PSRAMB_TZSRG_R0_OFFSET + region * 4, tmpVal);
...
void Tzc_Sec_Set_Se_Ctrl_Mode(TZC_SEC_SE_Ctrl_Mode mode)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_0);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SE_TZSID_CRMD, mode);
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_0, tmpVal);

    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_2);
    /* set lock */
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SE_TZSID_CRMD_LOCK, 1);
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_2, tmpVal);
}
...
void Tzc_Sec_Set_Sf_Ctrl_Mode(TZC_SEC_SF_Ctrl_Mode mode)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_1);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SF_TZSID_CRMD, mode);
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_1, tmpVal);

    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_2);
    /* set lock */
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SF_TZSID_CRMD_LOCK, 1);
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_2, tmpVal);
}
...
void Tzc_Sec_Set_Se_Group(TZC_SEC_SE_Ctrl_Type slaveType, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_0);
    /* set group */
    tmpVal &= (~(3 << (slaveType * 2)));
    tmpVal |= (group << (slaveType * 2));
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_0, tmpVal);
...
void Tzc_Sec_Set_Sf_Group(TZC_SEC_SF_Ctrl_Type slaveType, uint8_t group)
{
    ...
    tmpVal = QC7XX_RD_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_1);
    /* set group */
    tmpVal &= (~(3 << (slaveType * 2)));
    tmpVal |= (group << (slaveType * 2));
    QC7XX_WR_REG(TZ1_BASE, TZC_SEC_TZC_SE_CTRL_1, tmpVal);
...
void Tzc_Sec_Set_WTD_Rst_Delay(uint16_t delayValue)
{
    ...
    tmpVal = QC7XX_RD_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_0);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, TZC_SEC_TZC_SE_WDT_DLY, delayValue);
    QC7XX_WR_REG(TZC_SEC_BASE, TZC_SEC_TZC_SE_CTRL_0, tmpVal);
}
