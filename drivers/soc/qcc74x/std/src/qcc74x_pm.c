 ...
 *
 *******************************************************************************/
static QC7XX_Err_Type ATTR_TCM_SECTION pm_clr_pds_gpio_int(void)
{
    uint32_t tmpVal = 0;
 ...
 *
 *******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Set_LDO11_SOC_Sstart_Delay(uint8_t delay)
{
    uint32_t tmpVal = 0;
    ...
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_DCDC_SSTART_TIME_AON, delay);
    QC7XX_WR_REG(AON_BASE, AON_DCDC_TOP_0, tmpVal);

    return SUCCESS;
}

QC7XX_Err_Type pm_pds_wakeup_src_en(uint32_t WakeupType)
{
...
QC7XX_Sts_Type pm_pds_get_wakeup_src(uint32_t WakeupType)
{
    if ((WakeupType < PDS_WAKEUP_BY_PDS_TIMER_EN_POS) || (WakeupType > PDS_WAKEUP_BY_WIFI_TBTT_IRQ_EN_POS)) {
        return INVALID;
    }
    ...
    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QC7XX_GET_REG_BITS_VAL(tmpVal, PDS_RO_PDS_WAKEUP_EVENT);
...
void pm_pds_mask_all_wakeup_src(void)
{
    ...
    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_WAKEUP_SRC_EN);
    QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_RF_DONE_INT_MASK);
    QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_WIFI_TBTT_SLEEP_IRQ_MASK);
    QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_WIFI_TBTT_WAKEUP_IRQ_MASK);
    QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);
}
                                        ...
                                        uint32_t sleep_time)
{
    ...
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_GPADC_GLOBAL_EN);
    ...
    /* clear HBN irq*/
    // QC7XX_WR_REG(HBN_BASE, HBN_IRQ_CLR, 0xffffffff);
    // QC7XX_WR_REG(HBN_BASE, HBN_IRQ_CLR, 0);
    ...
    /* get flash config */
    qc7xx_flash_get_cfg((uint8_t **)&flash_cfg, &flash_cfg_len);
    ...
    if (0 != QC7XX_RD_REG(PDS_BASE, PDS_GPIO_I_SET)) {
        pPdsCfg->pdsCtl.ctrlGpioIePuPd = 1;
    }

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
    /* if enable wakeup trigger by irrx */
    if (QC7XX_IS_REG_BIT_SET(tmpVal, PDS_WAKEUP_BY_IRRX_EN)) {
   ...
   * ldo11soc_rdy_aon */
    ...
    pds_ram[0] = QC7XX_RD_REG(PDS_BASE, PDS_RAM1);
    pds_ram[1] = QC7XX_RD_REG(PDS_BASE, PDS_RAM2);
    pds_ram[2] = QC7XX_RD_REG(PDS_BASE, PDS_RAM3);
    pds_ram[3] = QC7XX_RD_REG(PDS_BASE, PDS_RAM4);
...
/******************************* Wakeup Flow *******************************/
    ...
    if (pds_level < PM_PDS_LEVEL_2) {
        /* Init flash gpio */
        qc7xx_sf_cfg_init_flash_gpio((uint8_t)sf_pin_select, 1);

        qc7xx_sf_ctrl_set_owner(SF_CTRL_OWNER_SAHB);
        qc7xx_sflash_restore_from_powerdown(flash_cfg, 0, SF_CTRL_FLASH_BANK0);
    }
#endif
                                        ...
                                        uint64_t sleep_time)
{
    ...
    qc7xx_irq_clear_pending(HBN_OUT0_IRQn);
    qc7xx_irq_clear_pending(HBN_OUT1_IRQn);

    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_CLR, 0xffffffff);
    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_CLR, 0);

    if (sleep_time) {
        struct qc7xx_device_s *rtc_dev = NULL;
        qc7xx_rtc_set_time(rtc_dev,sleep_time); // sleep time,unit is cycle
    } else {
    ...
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, HBN_LDO11_AON_VOUT_SEL, PM_HBN_LDO_LEVEL_DEFAULT);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, HBN_LDO11_RT_VOUT_SEL, PM_HBN_LDO_LEVEL_DEFAULT);
    ...
    switch (hbn_level) {
        case PM_HBN_LEVEL_0:
            ...
            tmpVal = QC7XX_CLR_REG_BIT(tmpVal, HBN_PWRDN_HBN_RTC);
            break;
        case PM_HBN_LEVEL_1:
        ...
        case PM_HBN_LEVEL_2:
            tmpVal = QC7XX_SET_REG_BIT(tmpVal, HBN_PWRDN_HBN_CORE);
            tmpVal = QC7XX_SET_REG_BIT(tmpVal, HBN_PWRDN_HBN_RTC);
            break;
    }
...
{
    QC7XX_WR_REG(HBN_BASE, HBN_RSV1, (uint32_t)wakeup_callback);
    /* Set HBN flag */
    QC7XX_WR_REG(HBN_BASE, HBN_RSV0, HBN_STATUS_ENTER_FLAG);
}
...
{
    QC7XX_WR_REG(HBN_BASE, HBN_RSV1, para);
    /* Set APP jump flag */
    QC7XX_WR_REG(HBN_BASE, HBN_RSV0, APP_JUMP_ENTER_FLAG);
}

void pm_hbn_out0_irq_register(void)
{
    qc7xx_irq_attach(HBN_OUT0_IRQn, (irq_callback)HBN_OUT0_IRQn, NULL);
    qc7xx_irq_enable(HBN_OUT0_IRQn);
}

void pm_hbn_out1_irq_register(void)
{
    qc7xx_irq_attach(HBN_OUT1_IRQn, (irq_callback)HBN_OUT1_IRQn, NULL);
    qc7xx_irq_enable(HBN_OUT1_IRQn);
}
...
void pm_rc32k_auto_cal(void)
{
    ...
    if(QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_F32K_SEL)){
        /* rtc use xtal32k, return */
        return;
    }

    /* disable gloabal irq */
    state=qc7xx_irq_save();
...
void hal_pm_ldo11_cfg(uint8_t soc_v, uint8_t rt_v, uint8_t aon_v)
{
    ...
    /* get current vol */
    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_GLB);
    soc_v_cur = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11SOC_VOUT_SEL_AON);
    rt_v_cur  = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_RT_VOUT_SEL);
    aon_v_cur = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_AON_VOUT_SEL);
    ...
    while((soc_v_cur != soc_v) || (rt_v_cur != rt_v) || (aon_v_cur != aon_v)) {
        ...
        /* write reg */
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11SOC_VOUT_SEL_AON, soc_v_cur);
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_RT_VOUT_SEL, rt_v_cur);
        tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_AON_VOUT_SEL, aon_v_cur);
...
void hal_pm_ldo11_cfg_get(uint8_t *soc_v, uint8_t *rt_v, uint8_t *aon_v)
{
    ...
    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_GLB);
    *soc_v = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11SOC_VOUT_SEL_AON);
    *rt_v  = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_RT_VOUT_SEL);
    *aon_v = QC7XX_GET_REG_BITS_VAL(tmpVal, HBN_SW_LDO11_AON_VOUT_SEL);
...
{
    qc7xx_irq_attach(PDS_WAKEUP_IRQn, (irq_callback)PDS_WAKEUP_IRQ, NULL);
    qc7xx_irq_enable(PDS_WAKEUP_IRQn);
}
