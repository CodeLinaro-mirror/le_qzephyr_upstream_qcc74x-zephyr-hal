...
#include "qcc74x_romapi_patch.h"
#include "qcc74x_romdriver_e907.h"
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_sflash_blk32_erase(spi_flash_cfg_type *flash_cfg, uint32_t blk_num)
{
    uint32_t cnt = 0;
    uint8_t is_32bits_addr = 0;
    struct sf_ctrl_cmd_cfg_type flash_cmd;
    int stat = qc7xx_sflash_write_enable(flash_cfg);
    ...
    if (is_32bits_addr > 0) {
        flash_cmd.addr_size++;
        flash_cmd.cmd_buf[0] = (flash_cfg->blk32_erase_cmd << 24) | ((QC7XX_SPIFLASH_BLK32K_SIZE * blk_num) >> 8);
        flash_cmd.cmd_buf[1] = ((QC7XX_SPIFLASH_BLK32K_SIZE * blk_num) << 24);
    } else {
        flash_cmd.cmd_buf[0] = (flash_cfg->blk32_erase_cmd << 24) | (QC7XX_SPIFLASH_BLK32K_SIZE * blk_num);
    }

    qc7xx_sf_ctrl_sendcmd(&flash_cmd);

    while (SET == qc7xx_sflash_busy(flash_cfg)) {
        arch_delay_us(500);
        cnt++;
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_sflash_erase(spi_flash_cfg_type *flash_cfg, uint32_t start_addr, uint32_t end_addr)
{
    uint32_t len = 0;
    ...
    while (start_addr <= end_addr) {
        ...
        if (flash_cfg->blk64_erase_cmd != QC7XX_SPIFLASH_CMD_INVALID &&
            (start_addr & (QC7XX_SPIFLASH_BLK64K_SIZE - 1)) == 0 &&
            len > (QC7XX_SPIFLASH_BLK64K_SIZE - flash_cfg->sector_size * 1024)) {
            /* 64K margin address,and length > 64K-sector size, erase one first */
            ret = qc7xx_sflash_blk64_erase(flash_cfg, start_addr / QC7XX_SPIFLASH_BLK64K_SIZE);
            erase_len = QC7XX_SPIFLASH_BLK64K_SIZE;
        } else if (flash_cfg->blk32_erase_cmd != QC7XX_SPIFLASH_CMD_INVALID &&
                   (start_addr & (QC7XX_SPIFLASH_BLK32K_SIZE - 1)) == 0 &&
                   len > (QC7XX_SPIFLASH_BLK32K_SIZE - flash_cfg->sector_size * 1024)) {
            /* 32K margin address,and length > 32K-sector size, erase one first */
            ret = qc7xx_sflash_blk32_erase(flash_cfg, start_addr / QC7XX_SPIFLASH_BLK32K_SIZE);
            erase_len = QC7XX_SPIFLASH_BLK32K_SIZE;
        } else {
            /* Sector erase */
            start_addr = ((start_addr) & (~(flash_cfg->sector_size * 1024 - 1)));
            ret = qc7xx_sflash_sector_erase(flash_cfg, start_addr / flash_cfg->sector_size / 1024);
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_SFlash_Clear_Status_Register(spi_flash_cfg_type *flash_cfg)
{
    uint32_t ret = 0;
    uint32_t qe_value = 0;
    ...
    qc7xx_sflash_read_reg(flash_cfg, 0, (uint8_t *)&read_reg_value0, 1);
    qc7xx_sflash_read_reg(flash_cfg, 1, (uint8_t *)&read_reg_value1, 1);
    ...
    ret = qc7xx_sflash_write_enable(flash_cfg);
    if (0 != ret) {
        return -1;
    }
    if (flash_cfg->qe_write_reg_len == 2) {
        reg_value = (qe_value << (flash_cfg->qe_index * 8 + flash_cfg->qe_bit));
        qc7xx_sflash_write_reg(flash_cfg, 0, (uint8_t *)&reg_value, 2);
    } else {
        ...
        qc7xx_sflash_write_reg(flash_cfg, 0, (uint8_t *)&reg_value, 1);
        ret = qc7xx_sflash_write_enable(flash_cfg);
        if (SUCCESS != ret) {
            return -1;
        }
        ...
        qc7xx_sflash_write_reg(flash_cfg, 1, (uint8_t *)&reg_value, 1);
    }
    return 0;
}
                                                             ...
                                                             uint8_t group, uint8_t bank)
{
    ...
    if (flash_id == 0) {
        ...
        qc7xx_xip_sflash_read_via_cache_need_lock(8 + QCC74X_FLASH_XIP_BASE, buf, sizeof(spi_flash_cfg_type) + 8, group, bank);
                                                         ...
                                                         uint8_t restore_default, spi_flash_cfg_type *p_flash_cfg, uint8_t group, uint8_t bank)
{
    ...
    ret = qc7xx_sf_cfg_flash_identify(call_from_flash, flash_pin_cfg, restore_default, p_flash_cfg, group, bank);
    if (call_from_flash) {
        qc7xx_sflash_set_xip_cfg(p_flash_cfg, p_flash_cfg->io_mode & 0xf, 1, 0, 32, bank);
    }
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_xip_sflash_getjedecid_need_lock(spi_flash_cfg_type *p_flash_cfg, uint8_t *data,
                                                          uint8_t group, uint8_t bank)
{
    int stat;
    uint32_t offset;
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_xip_sflash_getdeviceid_need_lock(spi_flash_cfg_type *p_flash_cfg, QC7XX_Fun_Type is_32bits_addr,
 ...
 *
*******************************************************************************/
int ATTR_TCM_SECTION qc7xx_xip_sflash_getuniqueid_need_lock(spi_flash_cfg_type *p_flash_cfg, uint8_t *data,
                                                           uint8_t idLen, uint8_t group, uint8_t bank)
{
    ...
    if (stat != SUCCESS) {
        ...
        qc7xx_sflash_get_uniqueid(data, idLen);
        qc7xx_xip_sflash_state_restore(p_flash_cfg, offset, group, bank);
    }
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION PDS_Set_GPIO_Pad_Pn_Pu_Pd_Ie(uint8_t grp, uint8_t pu, uint8_t pd, uint8_t ie)
{
    uint32_t tmpVal;
    uint32_t tmpValPu;
    uint32_t tmpValPd;
    ...
    /* pu/pd/ie config */
    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_GPIO_I_SET);
    tmpValPu = QC7XX_GET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_PU_SET);
    if (pu) {
    ...
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_PU_SET, tmpValPu);
    tmpValPd = QC7XX_GET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_PD_SET);
    if (pd) {
        tmpValPd |= (1 << grp);
    } else {
        tmpValPd &= ~(1 << grp);
    }
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_PD_SET, tmpValPd);
    tmpValIe = QC7XX_GET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_IE_SET);
    if (ie) {
        tmpValIe |= (1 << grp);
    } else {
        tmpValIe &= ~(1 << grp);
    }
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, PDS_CR_PDS_GPIO_IE_SET, tmpValIe);
    QC7XX_WR_REG(PDS_BASE, PDS_GPIO_I_SET, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_CLOCK_SECTION PDS_Pu_PLL_Enable(void)
{
    uint32_t tmpVal = 0;

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_PU_RST_CLKPLL);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_PU_CLKPLL_SFREG);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_PU_CLKPLL);
    QC7XX_WR_REG(PDS_BASE, PDS_PU_RST_CLKPLL, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_CLOCK_SECTION PDS_Pu_PLL_Disable(void)
{
    uint32_t tmpVal = 0;

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_PU_RST_CLKPLL);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_PU_CLKPLL_SFREG);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_PU_CLKPLL);
    QC7XX_WR_REG(PDS_BASE, PDS_PU_RST_CLKPLL, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION PDS_Enable(PDS_CTL_Type *cfg, PDS_CTL4_Type *cfg4, uint32_t pdsSleepCnt)
{
    uint32_t tmpVal = 0;
    ...
    if ((pdsSleepCnt) && (pdsSleepCnt <= PDS_WARMUP_LATENCY_CNT)) {
        ...
        QC7XX_WR_REG(PDS_BASE, PDS_TIME1, pdsSleepCnt - PDS_WARMUP_LATENCY_CNT);
    } else {
    }

    /* PDS_CTL4 config */
    QC7XX_WR_REG(PDS_BASE, PDS_CTL4, *(uint32_t *)cfg4);

    /* PDS_CTL config */
    if (cfg->pdsStart) {
        /* clear pds int */
        tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
        tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_INT_CLR);
        QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);

        tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_INT);
        tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_INT_CLR);
        QC7XX_WR_REG(PDS_BASE, PDS_INT, tmpVal);

        QC7XX_WR_REG(PDS_BASE, PDS_CTL, (*(uint32_t *)cfg & ~(1 << 0)));
        QC7XX_WR_REG(PDS_BASE, PDS_CTL, (*(uint32_t *)cfg | (1 << 0)));
    } else {
        QC7XX_WR_REG(PDS_BASE, PDS_CTL, *(uint32_t *)cfg);
    }
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION PDS_Default_Level_Config(PDS_DEFAULT_LV_CFG_Type *defaultLvCfg, uint32_t pdsSleepCnt)
{
 ...
 *
*******************************************************************************/
QC7XX_Err_Type PDS_Reset_Low_Power_USB_Ctrl(void)
{
    uint32_t tmpVal = 0;
    ...
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_CTRL_USBPLL_PD);
    ...
    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_CTL3);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_USB_ISO_EN);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL3, tmpVal);

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_CTL4);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_USB_PWR_OFF);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_USB_RESET);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL4, tmpVal);

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_CTL5);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_CTRL_USB33);
...
void ATTR_TCM_SECTION HBN_Power_Down_Flash(spi_flash_cfg_type *flashCfg)
{
    ...
    if (flashCfg == NULL) {
        ...
        qc7xx_xip_sflash_read_via_cache_need_lock(QCC74X_FLASH_XIP_BASE + 8 + 4, (uint8_t *)(&bhFlashCfg), sizeof(spi_flash_cfg_type), 0, 0);
        L1C_DCache_Invalid_By_Addr(QCC74X_FLASH_XIP_BASE + 8 + 4, sizeof(spi_flash_cfg_type));
 ...
 *
*******************************************************************************/
QC7XX_Err_Type PDS_Power_Off_WB(void)
{
    uint32_t tmpVal = 0;

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_CTL2);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_FORCE_WB_ISO_EN);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL2, tmpVal);

    tmpVal = QC7XX_SET_REG_BIT(tmpVal, PDS_CR_PDS_FORCE_WB_PWR_OFF);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL2, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type PDS_Power_On_WB(void)
{
    uint32_t tmpVal = 0;

    tmpVal = QC7XX_RD_REG(PDS_BASE, PDS_CTL2);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_FORCE_WB_PWR_OFF);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL2, tmpVal);

    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, PDS_CR_PDS_FORCE_WB_ISO_EN);
    QC7XX_WR_REG(PDS_BASE, PDS_CTL2, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type HBN_Disable_AComp0_IRQ(void)
{
    uint32_t tmpVal;

    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_IRQ_MODE);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, HBN_IRQ_ACOMP0_EN);
    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_MODE, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type HBN_Disable_AComp1_IRQ(void)
{
    uint32_t tmpVal;

    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_IRQ_MODE);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, HBN_IRQ_ACOMP1_EN);
    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_MODE, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type HBN_Enable_AComp0_IRQ(void)
{
    uint32_t tmpVal;

    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_IRQ_MODE);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, HBN_IRQ_ACOMP0_EN);
    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_MODE, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type HBN_Enable_AComp1_IRQ(void)
{
    uint32_t tmpVal;

    tmpVal = QC7XX_RD_REG(HBN_BASE, HBN_IRQ_MODE);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, HBN_IRQ_ACOMP1_EN);
    QC7XX_WR_REG(HBN_BASE, HBN_IRQ_MODE, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION PDS_Set_GPIO_Pad_IntMask(uint8_t pad, QC7XX_Mask_Type intMask)
{
    uint32_t tmpVal = 0;
    uint32_t pos = 0;
 ...
 *
*******************************************************************************/
QC7XX_Sts_Type ATTR_TCM_SECTION PDS_Get_GPIO_Pad_IntStatus(uint8_t pad)
{
    uint32_t pos = 0;
    ...
    return (QC7XX_RD_REG(PDS_BASE, PDS_GPIO_STAT) & (1 << pos)) ? SET : RESET;
}
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Output_Float_LDO15_RF(void)
{
    uint32_t tmpVal=0;

    /* ldo15rf pulldown select by aon */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_LDO15RF_PULLDOWN_SEL_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);

    /* ldo15rf float output */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_LDO15RF_PULLDOWN_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Output_Pulldown_LDO15_RF(void)
{
    uint32_t tmpVal=0;
    ...
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_LDO15RF_PULLDOWN_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Output_LDO15_RF_Ctrl_by_Pd(void)
{
    uint32_t tmpVal=0;

    /* output ldo15rf pulldown select by pd signal */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_LDO15RF_PULLDOWN_SEL_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Power_On_MicBias(void)
{
    uint32_t tmpVal = 0;

    /* power on sfreg */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_GPADC_REG_CMD);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_GPADC_MICBIAS_EN);
    QC7XX_WR_REG(AON_BASE, AON_GPADC_REG_CMD, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Power_Off_MicBias(void)
{
    uint32_t tmpVal = 0;

    /* power off sfreg */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_GPADC_REG_CMD);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_GPADC_MICBIAS_EN);
    QC7XX_WR_REG(AON_BASE, AON_GPADC_REG_CMD, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_LowPower_Enter_PDS0(void)
{
    uint32_t tmpVal = 0;

    /* power off sfreg */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_MISC);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_SW_WB_EN_AON);
    QC7XX_WR_REG(AON_BASE, AON_MISC, tmpVal);

    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_PU_SFREG_AON);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_PU_LDO15RF_AON);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_PU_MBG_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);

    /* gating Clock */
    tmpVal = QC7XX_RD_REG(GLB_BASE, GLB_CGEN_CFG0);
    tmpVal = tmpVal & (~(1 << 6));
    tmpVal = tmpVal & (~(1 << 7));
    QC7XX_WR_REG(GLB_BASE, GLB_CGEN_CFG0, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_LowPower_Exit_PDS0(void)
{
    uint32_t tmpVal = 0;

    tmpVal = QC7XX_RD_REG(AON_BASE, AON_RF_TOP_AON);

    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_PU_MBG_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);

    arch_delay_us(20);

    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_PU_LDO15RF_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);

    arch_delay_us(60);

    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_PU_SFREG_AON);
    QC7XX_WR_REG(AON_BASE, AON_RF_TOP_AON, tmpVal);
    ...
    /* power on wb */
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_MISC);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_SW_WB_EN_AON);
    QC7XX_WR_REG(AON_BASE, AON_MISC, tmpVal);
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Output_Pulldown_DCDC18(void)
{
    uint32_t tmpVal=0;

    tmpVal = QC7XX_RD_REG(AON_BASE, AON_DCDC_TOP_1);
    tmpVal = QC7XX_SET_REG_BIT(tmpVal, AON_DCDC_PULLDOWN_AON );
    QC7XX_WR_REG(AON_BASE, AON_DCDC_TOP_1, tmpVal);
    return SUCCESS;
}
 ...
 *
*******************************************************************************/
QC7XX_Err_Type ATTR_TCM_SECTION AON_Output_Float_DCDC18(void)
{
    uint32_t tmpVal=0;

    tmpVal = QC7XX_RD_REG(AON_BASE, AON_DCDC_TOP_1);
    tmpVal = QC7XX_CLR_REG_BIT(tmpVal, AON_DCDC_PULLDOWN_AON );
    QC7XX_WR_REG(AON_BASE, AON_DCDC_TOP_1, tmpVal);
