...
#if defined(RFPARAM_QCC74X)
#include "qcc74x_aon.h"
#include "qcc74x_mfg_media.h"
#include "qcc74x_hbn.h"
#endif

#if defined(RFPARAM_QCC75X)
#include "qcc75x_aon.h"
#include "qcc75x_hbn.h"
#endif

#define DBG_TAG "rfparam"
// #include "log.h"

// QC7XXLOG_DEFINE_TAG(RFPARAM, DBG_TAG, true);

#undef QCCLOG_TAG
#define QCCLOG_TAG           QCCLOG_GET_TAG(RFPARAM)

// #define rfparam_printf(...) LOG_I(__VA_ARGS__)
#define rfparam_printf(...) printf(__VA_ARGS__)
...
void rfparam_set_capcode(uint8_t capcode_in, uint8_t capcode_out)
{
    uint32_t tmpVal = 0;
#if defined(QCC75X)
    tmpVal = QC7XX_RD_REG(AON_BASE, AON_XTAL_CORE0);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_CAPCODE_IN_AON, capcode_in);
    tmpVal = QC7XX_SET_REG_BITS_VAL(tmpVal, AON_XTAL_CAPCODE_OUT_AON, capcode_out);
    QC7XX_WR_REG(AON_BASE, AON_XTAL_CORE0, tmpVal);
#elif defined(QCC74X)
...
#define WL_API_RMEM_ADDR 0x20010600
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "dcdc_trim", &trim, 1);
    if (trim.empty) {
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "icx", &trim, 1);
    if (trim.empty) {
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "iptat", &trim, 1);
    if (trim.empty) {
    ...
    qc7xx_ef_ctrl_read_common_trim(NULL, "tmp_mp2", &trim, 1);
    if (trim.en == 1 && trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
        g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
        rfparam_printf("tmp_mp2 value %d\r\n", (int)trim.value);
    } else {
        qc7xx_ef_ctrl_read_common_trim(NULL, "tmp_mp1", &trim, 1);
        if (trim.en == 1 && trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
            g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
            rfparam_printf("tmp_mp1 value %d\r\n", (int)trim.value);
        } else {
            qc7xx_ef_ctrl_read_common_trim(NULL, "tmp_mp0", &trim, 1);
            if (trim.en == 1 && trim.parity == qc7xx_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
                g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
