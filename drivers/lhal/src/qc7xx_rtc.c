...
#if defined(QCC74X)
#define QC7XX_RTC_BASE 0x2000F000
#elif defined(QCC75X)
#define QC7XX_RTC_BASE 0x2008F000
#endif

void qc7xx_rtc_disable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_rtc_disable
    romapi_qc7xx_rtc_disable(dev);
#else
    uint32_t reg_base;
    uint32_t regval;

    reg_base = QC7XX_RTC_BASE;
...
{
#ifdef romapi_qc7xx_rtc_set_time
    romapi_qc7xx_rtc_set_time(dev, time);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint64_t rtc_cnt;
...
{
#ifdef romapi_qc7xx_rtc_get_time
    return romapi_qc7xx_rtc_get_time(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint64_t time_l;
...
static struct qc7xx_tm *__gmtime_r(const time_t *timer, struct qc7xx_tm *result)
{
    time_t epoch;
    time_t jdn;
    int year;
...
static time_t __mktime(struct qc7xx_tm *tp)
{
    time_t ret;
    time_t jdn;
...
#define QC7XX_RTC_COUNTER_TO_MS(CNT) ((uint64_t)(CNT) * 1000 / 32768) // ((CNT)*(1024-16-8)/32768)
#define QC7XX_RTC_MAX_COUNTER        (0x000000FFFFFFFFFFllu)

uint64_t qc7xx_rtc_get_delta_counter(uint64_t ref_cnt)
{
    uint64_t cnt;

    cnt = qc7xx_rtc_get_time(NULL);
    ref_cnt &= QC7XX_RTC_MAX_COUNTER;

    if (cnt < ref_cnt) {
        cnt += QC7XX_RTC_MAX_COUNTER + 1;
    }

    return cnt - ref_cnt;
}

uint64_t qc7xx_rtc_get_delta_time_ms(uint64_t ref_cnt)
{
    uint64_t cnt;

    cnt = qc7xx_rtc_get_delta_counter(ref_cnt);

    return QC7XX_RTC_COUNTER_TO_MS(cnt);
}

static volatile struct qc7xx_tm g_rtc_tm;
static volatile uint64_t s_rtc_ref_cnt = 0;

void qc7xx_rtc_set_utc_time(const struct qc7xx_tm *time)
{
    arch_memcpy((void *)&g_rtc_tm, time, sizeof(struct qc7xx_tm));
    s_rtc_ref_cnt = qc7xx_rtc_get_time(NULL);
}
...
void qc7xx_rtc_get_utc_time(struct qc7xx_tm *time)
{
    ...
    time_stamp_ms = qc7xx_rtc_get_delta_time_ms(s_rtc_ref_cnt);
    time_stamp_ms = time_stamp_ms / 1000;
    time_stamp_ms += __mktime((struct qc7xx_tm *)&g_rtc_tm);
