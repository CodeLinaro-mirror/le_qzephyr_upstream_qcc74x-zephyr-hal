...
struct qc7xx_dma_irq_callback dma_callback[1][4];
#elif defined(QCC75X)
const uint32_t dma_base[] = { 0x20081000, 0x20086000, 0x000C0000 };
struct qc7xx_dma_irq_callback dma_callback[3][8];
#endif
...
void qc7xx_dma_channel_init(struct qc7xx_device_s *dev, const struct qc7xx_dma_channel_config_s *config)
{
...
#ifdef romapi_qc7xx_dma_channel_init
    romapi_qc7xx_dma_channel_init(dev, config);
#else
    uint32_t regval;
...
void qc7xx_dma_channel_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_deinit
    romapi_qc7xx_dma_channel_deinit(dev);
#else
    qc7xx_dma_channel_stop(dev);
    qc7xx_dma_channel_tcint_mask(dev, true);
#endif
}

__UNUSED static void qc7xx_dma_lli_config(struct qc7xx_device_s *dev,
                         struct qc7xx_dma_channel_lli_pool_s *lli_pool,
                         uint32_t lli_count,
                         uint32_t src_addr,
                         uint32_t dst_addr,
...
{
#ifdef romapi_qc7xx_dma_lli_config
    romapi_qc7xx_dma_lli_config(dev,
                               lli_pool,
                               lli_count,
                               src_addr,
                               dst_addr,
...
#else
    uint32_t channel_base;
    union qc7xx_dma_lli_control_s dma_ctrl_cfg;

    channel_base = dev->reg_base;

    dma_ctrl_cfg = (union qc7xx_dma_lli_control_s)getreg32(channel_base + DMA_CxCONTROL_OFFSET);
...
int qc7xx_dma_channel_lli_reload(struct qc7xx_device_s *dev, struct qc7xx_dma_channel_lli_pool_s *lli_pool, uint32_t max_lli_count, struct qc7xx_dma_channel_lli_transfer_s *transfer, uint32_t count)
{
#ifdef romapi_qc7xx_dma_channel_lli_reload
    return romapi_qc7xx_dma_channel_lli_reload(dev, lli_pool, max_lli_count, transfer, count);
#else
    uint32_t channel_base;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        qc7xx_dma_lli_config(dev, &lli_pool[lli_count_used_offset], current_lli_count, transfer[i].src_addr, transfer[i].dst_addr, actual_transfer_offset, last_transfer_len);
        if (i) {
    ...
    qc7xx_l1c_dcache_clean_range((uint32_t *)(uintptr_t)lli_pool, sizeof(struct qc7xx_dma_channel_lli_pool_s) * lli_count_used_offset);
#endif
    return lli_count_used_offset;
#endif
}

void qc7xx_dma_channel_lli_link_head(struct qc7xx_device_s *dev,
                                    struct qc7xx_dma_channel_lli_pool_s *lli_pool,
                                    uint32_t used_lli_count)
{
#ifdef romapi_qc7xx_dma_channel_lli_link_head
    romapi_qc7xx_dma_channel_lli_link_head(dev, lli_pool, used_lli_count);
#else
    uint32_t channel_base;
...
#if defined(QCC74X) || defined(QCC75X)
    /* clean cache, DMA does not pass through the cache */
    qc7xx_l1c_dcache_clean_range((uint32_t *)lli_pool, sizeof(struct qc7xx_dma_channel_lli_pool_s) * used_lli_count);
#endif
#endif
}

void qc7xx_dma_channel_start(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_start
    romapi_qc7xx_dma_channel_start(dev);
#else
    uint32_t regval;
    uint32_t channel_base;
...
void qc7xx_dma_channel_stop(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_stop
    romapi_qc7xx_dma_channel_stop(dev);
#else
    uint32_t regval;
    uint32_t channel_base;
...
bool qc7xx_dma_channel_isbusy(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_isbusy
    return romapi_qc7xx_dma_channel_isbusy(dev);
#else
    uint32_t regval;
    uint32_t channel_base;
...
void qc7xx_dma_channel_tcint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_dma_channel_tcint_mask
    romapi_qc7xx_dma_channel_tcint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t channel_base;
...
void qc7xx_dma_channel_irq_attach(struct qc7xx_device_s *dev, void (*callback)(void *arg), void *arg)
{
    dma_callback[dev->idx][dev->sub_idx].handler = callback;
    dma_callback[dev->idx][dev->sub_idx].arg = arg;

    qc7xx_dma_channel_tcint_mask(dev, false);

    if (dev->idx == 0) {
        qc7xx_irq_attach(dev->irq_num, dma0_isr, NULL);
        qc7xx_irq_enable(dev->irq_num);
    }
#if (0) && (defined(CPU_M0) || defined(CPU_LP)) || defined(QCC75X)
    else if (dev->idx == 1) {
        qc7xx_irq_attach(dev->irq_num, dma1_isr, NULL);
        qc7xx_irq_enable(dev->irq_num);
    }
#endif
#if ((0) && defined(CPU_D0)) || defined(QCC75X)
    else if (dev->idx == 2) {
        qc7xx_irq_attach(dev->irq_num, dma2_isr, NULL);
        qc7xx_irq_enable(dev->irq_num);
    }
#endif
}

void qc7xx_dma_channel_irq_detach(struct qc7xx_device_s *dev)
{
...
bool qc7xx_dma_channel_get_tcint_status(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_get_tcint_status
    return romapi_qc7xx_dma_channel_get_tcint_status(dev);
#else
    uint32_t regval;
...
void qc7xx_dma_channel_tcint_clear(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dma_channel_tcint_clear
    romapi_qc7xx_dma_channel_tcint_clear(dev);
#else
    putreg32(1 << dev->sub_idx, dma_base[dev->idx] + DMA_INTTCCLEAR_OFFSET);
#endif
}

int qc7xx_rx_cycle_dma_init(struct qc7xx_rx_cycle_dma *rx_dma,
                           struct qc7xx_device_s *dma_ch,
                           struct qc7xx_dma_channel_lli_pool_s *rx_llipool,
...
{
#ifdef romapi_qc7xx_rx_cycle_dma_init
    return romapi_qc7xx_rx_cycle_dma_init(rx_dma,
                                         dma_ch,
                                         rx_llipool,
                                         rx_llipool_size,
                                         src_addr,
                                         ...
                                         copy);
#else
    struct qc7xx_dma_channel_lli_transfer_s rx_transfers[2];
    ...
    int used_count = qc7xx_dma_channel_lli_reload(dma_ch, rx_llipool, rx_llipool_size, rx_transfers, 2);

    if (used_count < 0) {
        return -1;
    }

    qc7xx_dma_channel_lli_link_head(dma_ch, rx_llipool, used_count);
    return 0;
#endif
}

void qc7xx_rx_cycle_dma_process(struct qc7xx_rx_cycle_dma *rx_dma, bool in_dma_isr)
{
#ifdef romapi_qc7xx_rx_cycle_dma_process
    romapi_qc7xx_rx_cycle_dma_process(rx_dma, in_dma_isr);
#else
    uint16_t length;
    uint32_t dma_lli_count;
    uint8_t *write_ptr;

    write_ptr = (uint8_t *)(uintptr_t)qc7xx_dma_feature_control(rx_dma->dma_ch, DMA_CMD_GET_LLI_DSTADDR, 0);
    dma_lli_count = qc7xx_dma_feature_control(rx_dma->dma_ch, DMA_CMD_GET_LLI_COUNT, 0);
...
int qc7xx_dma_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_dma_feature_control
    return romapi_qc7xx_dma_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t regval;
    uint32_t channel_base;
