...
void qc7xx_cam_init(struct qc7xx_device_s *dev, const struct qc7xx_cam_config_s *config)
{
...
#ifdef romapi_qc7xx_cam_init
    romapi_qc7xx_cam_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint8_t data_mode = 0;
    ...
    putreg32(regval, CAM_FRONT_BASE + CAM_FRONT_DVP_MUX_SEL_REG_OFFSET);
        ...
        regval = qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_CAMERA, 0) / 1000000;
    ...
    qc7xx_cam_int_mask(dev, CAM_INTMASK_FIFO_OVERWRITE, true);
...
void qc7xx_cam_start(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_cam_start
    romapi_qc7xx_cam_start(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_cam_stop(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_cam_stop
    romapi_qc7xx_cam_stop(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_cam_int_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_cam_int_mask
    romapi_qc7xx_cam_int_mask(dev, int_type, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_cam_int_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_cam_int_clear
    romapi_qc7xx_cam_int_clear(dev, int_type);
#else
    putreg32(int_type, dev->reg_base + CAM_DVP_FRAME_FIFO_POP_OFFSET);
#endif
}

void qc7xx_cam_crop_vsync(struct qc7xx_device_s *dev, uint16_t start_line, uint16_t end_line)
{
#ifdef romapi_qc7xx_cam_crop_vsync
    romapi_qc7xx_cam_crop_vsync(dev, start_line, end_line);
#else
...
void qc7xx_cam_crop_hsync(struct qc7xx_device_s *dev, uint16_t start_pixel, uint16_t end_pixel)
{
#ifdef romapi_qc7xx_cam_crop_hsync
    romapi_qc7xx_cam_crop_hsync(dev, start_pixel, end_pixel);
#else
...
void qc7xx_cam_pop_one_frame(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_cam_pop_one_frame
    romapi_qc7xx_cam_pop_one_frame(dev);
#else
#if !defined(QCC75X)
...
uint8_t qc7xx_cam_get_frame_count(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_cam_get_frame_count
    return romapi_qc7xx_cam_get_frame_count(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
uint32_t qc7xx_cam_get_frame_info(struct qc7xx_device_s *dev, uint8_t **pic)
{
#ifdef romapi_qc7xx_cam_get_frame_info
    return romapi_qc7xx_cam_get_frame_info(dev, pic);
#else
    uint32_t reg_base;
...
uint32_t qc7xx_cam_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_cam_get_intstatus
    return romapi_qc7xx_cam_get_intstatus(dev);
#else
    return (getreg32(dev->reg_base + CAM_DVP_STATUS_AND_ERROR_OFFSET));
#endif
}

int qc7xx_cam_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_cam_feature_control
    return romapi_qc7xx_cam_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
    ...
    switch (cmd) {
        ...
        case CAM_CMD_FRAME_FILTER: {
            ...
            struct qc7xx_cam_frame_filter_config_s *config = (struct qc7xx_cam_frame_filter_config_s *)arg;
