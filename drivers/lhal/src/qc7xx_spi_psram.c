...
#include "qc7xx_spi_psram.h"
#include "qc7xx_l1c.h"

/** @addtogroup  QC7XX_Peripheral_Driver
 *  @{
 */
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_psram_init(struct spi_psram_cfg_type *psram_cfg, struct sf_ctrl_cmds_cfg *cmds_cfg,
                                      struct sf_ctrl_psram_cfg *sf_ctrl_psram_cfg)
{
    qc7xx_sf_ctrl_psram_init(sf_ctrl_psram_cfg);
    qc7xx_sf_ctrl_cmds_set(cmds_cfg, 0);

    qc7xx_psram_setdrivestrength(psram_cfg);
    qc7xx_psram_setburstwrap(psram_cfg);
}
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_psram_readreg(struct spi_psram_cfg_type *psram_cfg, uint8_t *reg_value)
{
    uint8_t *const psram_ctrl_buf = (uint8_t *)QC7XX_SF_CTRL_BASE;
    uint32_t timeout = 0;
    ...
    qc7xx_sf_ctrl_sendcmd(&psram_cmd);
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_psram_writereg(struct spi_psram_cfg_type *psram_cfg, uint8_t *reg_value)
{
    uint8_t *const psram_ctrl_buf = (uint8_t *)QC7XX_SF_CTRL_BASE;
    struct sf_ctrl_cmd_cfg_type psram_cmd;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_setdrivestrength(struct spi_psram_cfg_type *psram_cfg)
{
    uint32_t stat = 0;

    qc7xx_psram_readreg(psram_cfg, (uint8_t *)&stat);
    ...
    qc7xx_psram_writereg(psram_cfg, (uint8_t *)&stat);
    /* Wait for write done */
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_setburstwrap(struct spi_psram_cfg_type *psram_cfg)
{
    uint32_t stat = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_psram_readid(struct spi_psram_cfg_type *psram_cfg, uint8_t *data)
{
    uint8_t *const psram_ctrl_buf = (uint8_t *)QC7XX_SF_CTRL_BASE;
    uint32_t timeout = 0;
    struct sf_ctrl_cmd_cfg_type psram_cmd;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_enterquadmode(struct spi_psram_cfg_type *psram_cfg)
{
    struct sf_ctrl_cmd_cfg_type psram_cmd;
    uint32_t timeout = 0;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_exitquadmode(struct spi_psram_cfg_type *psram_cfg)
{
    struct sf_ctrl_cmd_cfg_type psram_cmd;
    uint32_t timeout = 0;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_toggleburstlength(struct spi_psram_cfg_type *psram_cfg, uint8_t ctrl_mode)
{
    struct sf_ctrl_cmd_cfg_type psram_cmd;
    uint32_t timeout = 0;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_softwarereset(struct spi_psram_cfg_type *psram_cfg, uint8_t ctrl_mode)
{
    struct sf_ctrl_cmd_cfg_type psram_cmd;
    uint32_t timeout = 0;
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_set_idbus_cfg(struct spi_psram_cfg_type *psram_cfg,
                                              uint8_t io_mode, uint32_t addr, uint32_t len)
{
    ...
    qc7xx_sf_ctrl_psram_read_set(&psram_cmd, cmd_valid);
    ...
    psram_cmd.dummy_clks = dummy_clks;
    psram_cmd.nb_data = len;
    qc7xx_sf_ctrl_psram_write_set(&psram_cmd, cmd_valid);
    return 0;
}
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_cache_write_set(struct spi_psram_cfg_type *psram_cfg, uint8_t io_mode,
                                                uint8_t wt_en, uint8_t wb_en, uint8_t wa_en)
{
    int stat = -1;

    /* Cache now only support 32 bytes read */
    stat = qc7xx_psram_set_idbus_cfg(psram_cfg, io_mode, 0, 32);

    if (0 != stat) {
        return stat;
    }

    qc7xx_l1c_cache_write_set(wt_en, wb_en, wa_en);
    return 0;
}
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_write(struct spi_psram_cfg_type *psram_cfg, uint8_t io_mode,
...
*******************************************************************************/
__WEAK
int ATTR_TCM_SECTION qc7xx_psram_read(struct spi_psram_cfg_type *psram_cfg, uint8_t io_mode,
                                     uint32_t addr, uint8_t *data, uint32_t len)
{
    uint8_t *const psram_ctrl_buf = (uint8_t *)QC7XX_SF_CTRL_BASE;
    uint32_t cur_len, i;
...
/*@} end of group QC7XX_Peripheral_Driver */
