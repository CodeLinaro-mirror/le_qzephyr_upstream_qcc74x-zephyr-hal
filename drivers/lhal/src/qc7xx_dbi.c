...
void qc7xx_dbi_init(struct qc7xx_device_s *dev, const struct qc7xx_dbi_config_s *config)
{
...
#ifdef romapi_qc7xx_dbi_init
    romapi_qc7xx_dbi_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    ...
    div = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_DBI, dev->idx) * 10 / config->clk_freq_hz + 5) / 10;
    div_0 = (div + 1) / 2;
...
void qc7xx_dbi_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dbi_deinit
    romapi_qc7xx_dbi_deinit(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
#if DBI_QSPI_SUPPORT
void qc7xx_dbi_qspi_set_addr(struct qc7xx_device_s *dev, uint8_t addr_byte_size, uint32_t addr_val)
{
#ifdef romapi_qc7xx_dbi_qspi_set_addr
    romapi_qc7xx_dbi_qspi_set_addr(dev, addr_byte_size, addr_val);
#else
    uint32_t reg_base;
    uint32_t regval;
...
__UNUSED static void qc7xx_dbi_fill_fifo(struct qc7xx_device_s *dev, uint32_t words_cnt, uint32_t *data_buff)
{
    uint32_t reg_base;
    uint32_t regval;
    uint32_t fifo_cnt;
...
__UNUSED static uint32_t qc7xx_dbi_get_words_cnt_form_pixel(struct qc7xx_device_s *dev, uint32_t pixle_cnt)
{
    uint32_t reg_base;
    uint32_t regval;
    uint32_t words_cnt;
...
int qc7xx_dbi_send_cmd_data(struct qc7xx_device_s *dev, uint8_t cmd, uint8_t data_len, uint8_t *data_buff)
{
#ifdef romapi_qc7xx_dbi_send_cmd_data
    return romapi_qc7xx_dbi_send_cmd_data(dev, cmd, data_len, data_buff);
#else
    uint32_t reg_base;
    uint32_t regval;
    ...
    if (data_len) {
        if (data_len <= DBI_FIFO_NUM_MAX * 4) {
            qc7xx_dbi_fill_fifo(dev, (data_len + 3) / 4, (uint32_t *)data_buff);
            data_len = 0;
        } else {
            qc7xx_dbi_fill_fifo(dev, DBI_FIFO_NUM_MAX, (uint32_t *)data_buff);
...
int qc7xx_dbi_send_cmd_read_data(struct qc7xx_device_s *dev, uint8_t cmd, uint8_t data_len, uint8_t *data_buff)
{
#ifdef romapi_qc7xx_dbi_send_cmd_read_data
    return romapi_qc7xx_dbi_send_cmd_read_data(dev, cmd, data_len, data_buff);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_dbi_send_cmd_pixel(struct qc7xx_device_s *dev, uint8_t cmd, uint32_t pixel_cnt, void *pixel_buff)
{
#ifdef romapi_qc7xx_dbi_send_cmd_pixel
    return romapi_qc7xx_dbi_send_cmd_pixel(dev, cmd, pixel_cnt, pixel_buff);
#else
    uint32_t reg_base;
    uint32_t regval;
    ...
    qc7xx_dbi_fill_fifo(dev, qc7xx_dbi_get_words_cnt_form_pixel(dev, pixel_cnt), (uint32_t *)pixel_buff);
...
void qc7xx_dbi_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_dbi_link_txdma
    romapi_qc7xx_dbi_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_dbi_txint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_dbi_txint_mask
    romapi_qc7xx_dbi_txint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_dbi_tcint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_dbi_tcint_mask
    romapi_qc7xx_dbi_tcint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_dbi_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_dbi_errint_mask
    romapi_qc7xx_dbi_errint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
uint32_t qc7xx_dbi_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_dbi_get_intstatus
    return romapi_qc7xx_dbi_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t int_sts;
...
void qc7xx_dbi_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
#ifdef romapi_qc7xx_dbi_int_clear
    romapi_qc7xx_dbi_int_clear(dev, int_clear);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_dbi_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_dbi_feature_control
    return romapi_qc7xx_dbi_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
    ...
    switch (cmd) {
        ...
        case DBI_CMD_GET_SIZE_OF_PIXEL_CNT:
            ...
            ret = qc7xx_dbi_get_words_cnt_form_pixel(dev, arg) * 4;
            break;
