...
__UNUSED static int qc7xx_adc_fifo1_is_enabled(void)
{
    uint32_t regval;
...
void qc7xx_adc_common_init(const struct qc7xx_adc_common_config_s *config)
{
#ifdef romapi_qc7xx_adc_common_init
    romapi_qc7xx_adc_common_init(config);
#else
    uint32_t regval;
    ...
    qc7xx_adc_int_watchdog_mask(NULL, ADC_INT_WATCHDOG_ALL, true);
    qc7xx_adc_int_watchdog_clear(NULL, ADC_INT_WATCHDOG_ALL);
#endif
}

void qc7xx_adc_init(struct qc7xx_device_s *dev, const struct qc7xx_adc_config_s *config)
{
#ifdef romapi_qc7xx_adc_init
    romapi_qc7xx_adc_init(dev, config);
#else
    uint32_t regval;
    uint32_t reg_base;
    ...
    qc7xx_adc_int_saturation_mask(dev, ADC_INT_SATURATION_ALL, true);
    qc7xx_adc_int_done_mask(dev, ADC_INT_DONE_ALL, true);
    qc7xx_adc_int_fifo_mask(dev, ADC_INT_FIFO_ALL, true);
    qc7xx_adc_int_saturation_clear(dev, ADC_INT_SATURATION_ALL);
    qc7xx_adc_int_done_clear(dev, ADC_INT_DONE_ALL);
    qc7xx_adc_int_fifo_clear(dev, ADC_INT_FIFO_ALL);

    if ((dev->idx == 1) && qc7xx_adc_fifo1_is_enabled()) {
...
void qc7xx_adc_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_deinit
    romapi_qc7xx_adc_deinit(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_adc_link_rxdma
    romapi_qc7xx_adc_link_rxdma(dev, enable);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
int qc7xx_adc_channel_config_internal(struct qc7xx_device_s *dev, struct qc7xx_adc_channel_s *chan, uint8_t channels)
{
#ifdef romapi_qc7xx_adc_channel_config_internal
    return romapi_qc7xx_adc_channel_config_internal(dev, chan, channels);
#else
    uint32_t regval;
    uint32_t reg_base;
...
int qc7xx_adc_channel_config_external(struct qc7xx_device_s *dev, struct qc7xx_adc_channel_s *chan, uint8_t channels)
{
#ifdef romapi_qc7xx_adc_channel_config_external
    return romapi_qc7xx_adc_channel_config_external(dev, chan, channels);
#else
    uint32_t regval;
    uint32_t regval2;
    uint32_t reg_base;
...
int qc7xx_adc_channel_config_internal_inject(struct qc7xx_device_s *dev, struct qc7xx_adc_channel_s *chan, uint8_t channels)
{
#ifdef romapi_qc7xx_adc_channel_config_internal_inject
    return romapi_qc7xx_adc_channel_config_internal_inject(dev, chan, channels);
#else
    uint32_t regval;
    uint32_t regval2;
    uint32_t reg_base;
...
int qc7xx_adc_channel_config_external_inject(struct qc7xx_device_s *dev, struct qc7xx_adc_channel_s *chan, uint8_t channels)
{
#ifdef romapi_qc7xx_adc_channel_config_external_inject
    return romapi_qc7xx_adc_channel_config_external_inject(dev, chan, channels);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_start_conversion(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_start_conversion
    romapi_qc7xx_adc_start_conversion(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_start_conversion_inject(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_start_conversion_inject
    romapi_qc7xx_adc_start_conversion_inject(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_stop_conversion(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_stop_conversion
    romapi_qc7xx_adc_stop_conversion(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_stop_conversion_inject(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_stop_conversion_inject
    romapi_qc7xx_adc_stop_conversion_inject(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
uint8_t qc7xx_adc_get_count(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_get_count
    return romapi_qc7xx_adc_get_count(dev);
#else
    if ((dev->idx == 1) && qc7xx_adc_fifo1_is_enabled()) {
...
uint8_t qc7xx_adc_get_count_inject(struct qc7xx_device_s *dev, uint8_t idx)
{
#ifdef romapi_qc7xx_adc_get_count_inject
    return romapi_qc7xx_adc_get_count_inject(dev, idx);
#else
    uint32_t regval;
...
uint32_t qc7xx_adc_read_raw(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_read_raw
    return romapi_qc7xx_adc_read_raw(dev);
#else
#if defined(QCC75X_VERSION_A0)
...
uint32_t qc7xx_adc_read_raw_inject(struct qc7xx_device_s *dev, uint8_t idx)
{
#ifdef romapi_qc7xx_adc_read_raw_inject
    return romapi_qc7xx_adc_read_raw_inject(dev, idx);
#else
    uint32_t reg_addr;
...
void qc7xx_adc_clear_fifo(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_clear_fifo
    romapi_qc7xx_adc_clear_fifo(dev);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
void qc7xx_adc_clear_fifo_inject(struct qc7xx_device_s *dev, uint8_t idx)
{
#ifdef romapi_qc7xx_adc_clear_fifo_inject
    romapi_qc7xx_adc_clear_fifo_inject(dev, idx);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
void qc7xx_adc_int_saturation_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_adc_int_saturation_mask
    romapi_qc7xx_adc_int_saturation_mask(dev, int_type, mask);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_int_watchdog_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_adc_int_watchdog_mask
    romapi_qc7xx_adc_int_watchdog_mask(dev, int_type, mask);
#else
    uint32_t regval;
...
void qc7xx_adc_int_done_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_adc_int_done_mask
    romapi_qc7xx_adc_int_done_mask(dev, int_type, mask);
#else
    uint32_t regval;
...
void qc7xx_adc_int_fifo_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_adc_int_fifo_mask
    romapi_qc7xx_adc_int_fifo_mask(dev, int_type, mask);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
uint32_t qc7xx_adc_get_int_saturation_status(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_get_int_saturation_status
    return romapi_qc7xx_adc_get_int_saturation_status(dev, int_type);
#else
    uint32_t regval;
    uint32_t reg_base;
...
uint32_t qc7xx_adc_get_int_watchdog_status(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_get_int_watchdog_status
    return romapi_qc7xx_adc_get_int_watchdog_status(dev, int_type);
#else
    uint32_t regval;
...
uint32_t qc7xx_adc_get_int_done_status(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_get_int_done_status
    return romapi_qc7xx_adc_get_int_done_status(dev, int_type);
#else
    uint32_t regval;
...
uint32_t qc7xx_adc_get_int_fifo_status(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_get_int_fifo_status
    return romapi_qc7xx_adc_get_int_fifo_status(dev, int_type);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
void qc7xx_adc_int_saturation_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_int_saturation_clear
    romapi_qc7xx_adc_int_saturation_clear(dev, int_type);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
void qc7xx_adc_int_watchdog_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_int_watchdog_clear
    romapi_qc7xx_adc_int_watchdog_clear(dev, int_type);
#else
    uint32_t regval;
...
void qc7xx_adc_int_done_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_int_done_clear
    romapi_qc7xx_adc_int_done_clear(dev, int_type);
#else
    uint32_t regval;
...
void qc7xx_adc_int_fifo_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_adc_int_fifo_clear
    romapi_qc7xx_adc_int_fifo_clear(dev, int_type);
#else
    uint32_t regval;
    uint32_t reg_addr;
...
void qc7xx_adc_parse_result(struct qc7xx_device_s *dev, uint32_t *buffer, struct qc7xx_adc_result_s *result, uint16_t count)
{
#ifdef romapi_qc7xx_adc_parse_result
    romapi_qc7xx_adc_parse_result(dev, buffer, result, count);
#else
    uint32_t reg_base;
    uint32_t ref = 1800;
...
void qc7xx_adc_tsen_init(struct qc7xx_device_s *dev, uint8_t tsen_mod)
{
#ifdef romapi_qc7xx_adc_tsen_init
    romapi_qc7xx_adc_tsen_init(dev, tsen_mod);
#else
    uint32_t regval;
    uint32_t reg_base;
...
float qc7xx_adc_tsen_get_temp(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_adc_tsen_get_temp
    return romapi_qc7xx_adc_tsen_get_temp(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
    uint32_t v0 = 0, v1 = 0;
    ...
    sum = 0;
    qc7xx_adc_start_conversion(dev);
    for (int i = 0; i < 8; i++) {
        start_time = qc7xx_mtimer_get_time_ms();
        while (qc7xx_adc_get_count(dev) == 0) {
            if ((qc7xx_mtimer_get_time_ms() - start_time) > 100) {
                return -ETIMEDOUT;
            }
        }
        qc7xx_adc_read_raw(dev);
    }
    for (int i = 0; i < 16; i++) {
        start_time = qc7xx_mtimer_get_time_ms();
        while (qc7xx_adc_get_count(dev) == 0) {
        ...
        raw_data = qc7xx_adc_read_raw(dev);
        sum += (raw_data & 0xFFFF);
    }
    qc7xx_adc_stop_conversion(dev);
    v0 = (sum + 8) / 16;
...
void qc7xx_adc_watchdog_init(struct qc7xx_device_s *dev, struct qc7xx_adc_watchdog_config_s *config)
{
#ifdef romapi_qc7xx_adc_watchdog_init
    romapi_qc7xx_adc_watchdog_init(dev, config);
#else
    uint32_t regval;
    uint32_t ch_sel;
...
int qc7xx_adc_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef qc7xx_adc_feature_control
    return qc7xx_adc_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t regval;
    uint32_t reg_base;
