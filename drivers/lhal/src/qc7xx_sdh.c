...
int qc7xx_sdh_init(struct qc7xx_device_s *dev, struct qc7xx_sdh_config_s *cfg)
{
    LHAL_PARAM_ASSERT(dev);
    LHAL_PARAM_ASSERT(cfg);

#ifdef romapi_qc7xx_sdh_init
    return romapi_qc7xx_sdh_init(dev, cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_sdh_adma2_desc_init(struct qc7xx_device_s *dev, struct qc7xx_sdh_data_cfg_s *data_cfg)
{
    LHAL_PARAM_ASSERT(dev);

#ifdef romapi_qc7xx_sdh_adma2_desc_init
    return romapi_qc7xx_sdh_adma2_desc_init(dev, data_cfg);
#else
    struct qc7xx_sdh_data_tranfer_s *curr_tranfer = NULL;
    struct qc7xx_sdh_adma2_hw_desc_s *curr_hw_desc = NULL;
...
/*  */
int qc7xx_sdh_cmd_cfg(struct qc7xx_device_s *dev, struct qc7xx_sdh_cmd_cfg_s *cmd_cfg)
{
    LHAL_PARAM_ASSERT(dev);
    LHAL_PARAM_ASSERT(cmd_cfg);

#ifdef romapi_qc7xx_sdh_cmd_cfg
    return romapi_qc7xx_sdh_cmd_cfg(dev, cmd_cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_sdh_data_cfg(struct qc7xx_device_s *dev, struct qc7xx_sdh_data_cfg_s *data_cfg)
{
    LHAL_PARAM_ASSERT(dev);

#ifdef romapi_qc7xx_sdh_data_cfg
    return romapi_qc7xx_sdh_data_cfg(dev, data_cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_sdh_tranfer_start(struct qc7xx_device_s *dev, struct qc7xx_sdh_cmd_cfg_s *cmd_cfg, struct qc7xx_sdh_data_cfg_s *data_cfg)
{
    LHAL_PARAM_ASSERT(dev);
    LHAL_PARAM_ASSERT(cmd_cfg);

#ifdef romapi_qc7xx_sdh_tranfer_start
    return romapi_qc7xx_sdh_tranfer_start(dev, cmd_cfg, data_cfg);
#else
    int ret;
    uint32_t reg_base;
    ...
    /* cmd cfg */
    ret = qc7xx_sdh_cmd_cfg(dev, cmd_cfg);
    if (ret < 0) {
        return ret;
    }
    ...
    if (data_cfg) {
        ret = qc7xx_sdh_adma2_desc_init(dev, data_cfg);
        if (ret < 0) {
            return ret;
        }
    }
    ...
    /* data cfg */
    ret = qc7xx_sdh_data_cfg(dev, data_cfg);
    if (ret < 0) {
        return ret;
    }
...
int qc7xx_sdh_get_resp(struct qc7xx_device_s *dev, struct qc7xx_sdh_cmd_cfg_s *cmd_cfg)
{
    LHAL_PARAM_ASSERT(dev);
    LHAL_PARAM_ASSERT(cmd_cfg);

#ifdef romapi_qc7xx_sdh_get_resp
    return romapi_qc7xx_sdh_get_resp(dev, cmd_cfg);
#else
    uint32_t reg_base;
    reg_base = dev->reg_base;
...
void qc7xx_sdh_sta_en(struct qc7xx_device_s *dev, uint32_t sta_bit, bool en)
{
#ifdef romapi_qc7xx_sdh_sta_en
    romapi_qc7xx_sdh_sta_en(dev, sta_bit, en);
#else
    uint32_t regval;
...
uint32_t qc7xx_sdh_sta_en_get(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_sdh_sta_en_get
    return romapi_qc7xx_sdh_sta_en_get(dev);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
    regval = getreg32(reg_base + SDH_NORMAL_INT_STATUS_EN_OFFSET);
...
void qc7xx_sdh_sta_int_en(struct qc7xx_device_s *dev, uint32_t sta_bit, bool en)
{
#ifdef romapi_qc7xx_sdh_sta_int_en
    romapi_qc7xx_sdh_sta_int_en(dev, sta_bit, en);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
uint32_t qc7xx_sdh_sta_int_en_get(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_sdh_sta_int_en_get
    return romapi_qc7xx_sdh_sta_int_en_get(dev);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
    regval = getreg32(reg_base + SDH_NORMAL_INT_STATUS_INT_EN_OFFSET);
...
/* get status */
uint32_t qc7xx_sdh_sta_get(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_sdh_sta_get
    return romapi_qc7xx_sdh_sta_get(dev);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
/* clr status */
void qc7xx_sdh_sta_clr(struct qc7xx_device_s *dev, uint32_t sta_bit)
{
#ifdef romapi_qc7xx_sdh_sta_clr
    romapi_qc7xx_sdh_sta_clr(dev, sta_bit);
#else
    uint32_t reg_base = dev->reg_base;
...
int qc7xx_sdh_feature_control(struct qc7xx_device_s *dev, int cmd, uintptr_t arg)
{
#ifdef romapi_qc7xx_sdh_feature_control
    return romapi_qc7xx_sdh_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
