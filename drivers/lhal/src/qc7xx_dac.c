...
void qc7xx_dac_init(struct qc7xx_device_s *dev, uint8_t clk_div)
{
    LHAL_PARAM_ASSERT(dev);
    LHAL_PARAM_ASSERT(IS_DAC_CLK_DIV(clk_div));

#ifdef romapi_qc7xx_dac_init
    romapi_qc7xx_dac_init(dev, clk_div);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_dac_channel_enable(struct qc7xx_device_s *dev, uint8_t ch)
{
#ifdef romapi_qc7xx_dac_channel_enable
    romapi_qc7xx_dac_channel_enable(dev, ch);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_dac_channel_disable(struct qc7xx_device_s *dev, uint8_t ch)
{
#ifdef romapi_qc7xx_dac_channel_disable
    romapi_qc7xx_dac_channel_disable(dev, ch);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_dac_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_dac_link_txdma
    romapi_qc7xx_dac_link_txdma(dev, enable);
#else
    uint32_t regval1;
    uint32_t regval2;
    uint32_t reg_base;
...
void qc7xx_dac_set_dma_format(struct qc7xx_device_s *dev, uint8_t format)
{
#ifdef romapi_qc7xx_dac_set_dma_format
    romapi_qc7xx_dac_set_dma_format(dev, format);
#else
    uint32_t regval;
...
void qc7xx_dac_set_value(struct qc7xx_device_s *dev, uint8_t ch, uint16_t value)
{
#ifdef romapi_qc7xx_dac_set_value
    romapi_qc7xx_dac_set_value(dev, ch, value);
#else
    uint32_t regval;
    uint32_t reg_base;
