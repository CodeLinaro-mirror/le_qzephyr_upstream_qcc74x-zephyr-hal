...
#define qc7xx_sf_ctrl_get_aes_region(addr, r) (addr + SF_CTRL_AES_REGION_OFFSET + (r)*0x80)
#define QC7XX_RDWD_FRM_BYTEP(p) ((p[3] << 24) | (p[2] << 16) | (p[1] << 8) | (p[0]))
...
__attribute__((always_inline)) __STATIC_INLINE uint32_t qc7xx_sf_ctrl_bswap32(uint32_t value)
{
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_enable(const struct sf_ctrl_cfg_type *cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_enable
    romapi_qc7xx_sf_ctrl_enable(cfg);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
    ...
    while (qc7xx_sf_ctrl_get_busy_state()) {
        time_out--;
    ...
    qc7xx_sf_ctrl_set_owner(cfg->owner);
#endif
}
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_io_delay(uint8_t pad, uint8_t do_delay, uint8_t di_delay, uint8_t oe_delay)
{
#ifdef romapi_qc7xx_sf_ctrl_set_io_delay
    romapi_qc7xx_sf_ctrl_set_io_delay(pad, do_delay, di_delay, oe_delay);
#else
    uint32_t offset = 0;
    uint32_t regval = 0;

    if (pad == SF_CTRL_PAD1) {
        offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF_IO_DLY_1_OFFSET;
    } else if (pad == SF_CTRL_PAD2) {
        offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF_IO_DLY_2_OFFSET;
    } else {
        offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF_IO_DLY_3_OFFSET;
    }
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_get_io_delay(uint8_t pad, uint8_t *do_delay, uint8_t *di_delay, uint8_t *oe_delay)
{
#ifdef romapi_qc7xx_sf_ctrl_get_io_delay
    romapi_qc7xx_sf_ctrl_get_io_delay(pad, do_delay, di_delay, oe_delay);
#else
    uint32_t offset = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_cs_clk_delay(uint8_t pad, uint8_t cs_delay, uint8_t clk_delay)
{
#ifdef romapi_qc7xx_sf_ctrl_set_cs_clk_delay
    romapi_qc7xx_sf_ctrl_set_cs_clk_delay(pad, cs_delay, clk_delay);
#else
    uint32_t offset = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_get_cs_clk_delay(uint8_t pad, uint8_t *cs_delay, uint8_t *clk_delay)
{
#ifdef romapi_qc7xx_sf_ctrl_get_cs_clk_delay
    romapi_qc7xx_sf_ctrl_get_cs_clk_delay(pad, cs_delay, clk_delay);
#else
    uint32_t offset = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_flash_io_cs_clk_delay(struct qc7xx_sf_ctrl_io_cs_clk_delay_cfg cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_set_flash_io_cs_clk_delay
    romapi_qc7xx_sf_ctrl_set_flash_io_cs_clk_delay(cfg);
#else
    /* Set do di and oe delay */
    qc7xx_sf_ctrl_set_io_delay(0, cfg.do_delay, cfg.di_delay, cfg.oe_delay);

    /* Set cs and clk delay */
    qc7xx_sf_ctrl_set_cs_clk_delay(0, cfg.cs_delay, cfg.cs_clk_delay);
#endif
}
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_get_flash_io_cs_clk_delay(struct qc7xx_sf_ctrl_io_cs_clk_delay_cfg *cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_get_flash_io_cs_clk_delay
    romapi_qc7xx_sf_ctrl_get_flash_io_cs_clk_delay(cfg);
#else
    arch_memset(cfg, 0, sizeof(struct qc7xx_sf_ctrl_io_cs_clk_delay_cfg));

    /* Get do di and oe delay */
    qc7xx_sf_ctrl_get_io_delay(0, &cfg->do_delay, &cfg->di_delay, &cfg->oe_delay);

    /* Get cs and clk delay */
    qc7xx_sf_ctrl_get_cs_clk_delay(0, &cfg->cs_delay, &cfg->cs_clk_delay);
#endif
}
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_dqs_delay(uint8_t pad, uint8_t dodelay, uint8_t didelay, uint8_t oedelay)
{
#ifdef romapi_qc7xx_sf_ctrl_set_dqs_delay
    romapi_qc7xx_sf_ctrl_set_dqs_delay(pad, dodelay, didelay, oedelay);
#else
    uint32_t offset = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_bank2_enable(const struct sf_ctrl_bank2_cfg *bank2_cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_bank2_enable
    romapi_qc7xx_sf_ctrl_bank2_enable(bank2_cfg);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus2_hold_sram(void)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus2_hold_sram
    romapi_qc7xx_sf_ctrl_sbus2_hold_sram();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus2_release_sram(void)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus2_release_sram
    romapi_qc7xx_sf_ctrl_sbus2_release_sram();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_is_sbus2_enable(void)
{
#ifdef romapi_qc7xx_sf_ctrl_is_sbus2_enable
    return romapi_qc7xx_sf_ctrl_is_sbus2_enable();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus2_replace(uint8_t pad)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus2_replace
    romapi_qc7xx_sf_ctrl_sbus2_replace(pad);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus2_revoke_replace(void)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus2_revoke_replace
    romapi_qc7xx_sf_ctrl_sbus2_revoke_replace();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    ...
    qc7xx_sf_ctrl_sbus2_release_sram();
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus2_set_delay(uint8_t clk_delay, uint8_t rx_clk_invert)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus2_set_delay
    romapi_qc7xx_sf_ctrl_sbus2_set_delay(clk_delay, rx_clk_invert);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_remap_set(uint8_t remap, uint8_t lock)
{
#ifdef romapi_qc7xx_sf_ctrl_remap_set
    romapi_qc7xx_sf_ctrl_remap_set(remap, lock);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_32bits_addr_en(uint8_t en32_bits_addr)
{
#ifdef romapi_qc7xx_sf_ctrl_32bits_addr_en
    romapi_qc7xx_sf_ctrl_32bits_addr_en(en32_bits_addr);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_psram_init(struct sf_ctrl_psram_cfg *psram_cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_psram_init
    romapi_qc7xx_sf_ctrl_psram_init(psram_cfg);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    ...
    qc7xx_sf_ctrl_set_owner(psram_cfg->owner);
#endif
}
#endif
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_clock_delay(void)
{
#ifdef romapi_qc7xx_sf_ctrl_get_clock_delay
    return romapi_qc7xx_sf_ctrl_get_clock_delay();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_clock_delay(uint8_t delay)
{
#ifdef romapi_qc7xx_sf_ctrl_set_clock_delay
    romapi_qc7xx_sf_ctrl_set_clock_delay(delay);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_wrap_queue_value(void)
{
#ifdef romapi_qc7xx_sf_ctrl_get_wrap_queue_value
    return romapi_qc7xx_sf_ctrl_get_wrap_queue_value();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_cmds_set(struct sf_ctrl_cmds_cfg *cmds_cfg, uint8_t bank)
{
#ifdef romapi_qc7xx_sf_ctrl_cmds_set
    return romapi_qc7xx_sf_ctrl_cmds_set(cmds_cfg, bank);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_select_pad(uint8_t sel)
{
#ifdef romapi_qc7xx_sf_ctrl_select_pad
    romapi_qc7xx_sf_ctrl_select_pad(sel);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sbus_select_bank(uint8_t bank)
{
#ifdef romapi_qc7xx_sf_ctrl_sbus_select_bank
    romapi_qc7xx_sf_ctrl_sbus_select_bank(bank);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_owner(uint8_t owner)
{
#ifdef romapi_qc7xx_sf_ctrl_set_owner
    romapi_qc7xx_sf_ctrl_set_owner(owner);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_owner(void)
{
#ifdef romapi_qc7xx_sf_ctrl_get_owner
    return romapi_qc7xx_sf_ctrl_get_owner();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_owner_flag(uint8_t owner)
{
#ifdef romapi_qc7xx_sf_ctrl_set_owner_flag
    romapi_qc7xx_sf_ctrl_set_owner_flag(owner);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_owner_flag(void)
{
#ifdef romapi_qc7xx_sf_ctrl_get_owner_flag
    return romapi_qc7xx_sf_ctrl_get_owner_flag();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint8_t owner = 0xFF;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_disable(void)
{
#ifdef romapi_qc7xx_sf_ctrl_disable
    romapi_qc7xx_sf_ctrl_disable();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_enable_be(void)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_enable_be
    romapi_qc7xx_sf_ctrl_aes_enable_be();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_enable_le(void)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_enable_le
    romapi_qc7xx_sf_ctrl_aes_enable_le();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_region(uint8_t region, uint8_t enable, uint8_t hw_key,
                                                  uint32_t start_addr, uint32_t end_addr, uint8_t locked)
{
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;

    if (!hw_key) {
        region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    }
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_key(uint8_t region, uint8_t *key, uint8_t key_type)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_key
    romapi_qc7xx_sf_ctrl_aes_set_key(region, key, key_type);
#else
    uint32_t reg_base = 0;
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    ...
    if (NULL != key) {
        ...
        while (i--) {
            putreg32(qc7xx_sf_ctrl_bswap32(QC7XX_RDWD_FRM_BYTEP(key)), region_reg_base + regval);
            key += 4;
            regval -= 4;
        }
    }
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_key_be(uint8_t region, uint8_t *key, uint8_t key_type)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_key_be
    romapi_qc7xx_sf_ctrl_aes_set_key_be(region, key, key_type);
#else
    uint32_t reg_base = 0;
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    ...
    if (NULL != key) {
        ...
        while (i--) {
            putreg32(QC7XX_RDWD_FRM_BYTEP(key), region_reg_base + regval);
            key += 4;
            regval += 4;
        }
    }
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_iv(uint8_t region, uint8_t *iv, uint32_t addr_offset)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_iv
    romapi_qc7xx_sf_ctrl_aes_set_iv(region, iv, addr_offset);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    uint32_t i = 3;
    ...
    if (iv != NULL) {
        ...
        while (i--) {
            putreg32(qc7xx_sf_ctrl_bswap32(QC7XX_RDWD_FRM_BYTEP(iv)), region_reg_base + regval);
            iv += 4;
            regval -= 4;
        }
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_iv_be(uint8_t region, uint8_t *iv, uint32_t addr_offset)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_iv_be
    romapi_qc7xx_sf_ctrl_aes_set_iv_be(region, iv, addr_offset);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    uint32_t i = 3;
    ...
    if (iv != NULL) {
        ...
        while (i--) {
            putreg32(QC7XX_RDWD_FRM_BYTEP(iv), region_reg_base + regval);
            iv += 4;
            regval += 4;
        }

        putreg32(qc7xx_sf_ctrl_bswap32(addr_offset), region_reg_base + SF_CTRL_SF_AES_IV_W3_OFFSET);
        iv += 4;
    }
#endif
}
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_region_offset(uint8_t region, uint32_t addr_offset)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_region_offset
    romapi_qc7xx_sf_ctrl_aes_set_region_offset(region, addr_offset);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_xts_set_key(uint8_t region, uint8_t *key, uint8_t key_type)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_xts_set_key
    romapi_qc7xx_sf_ctrl_aes_xts_set_key(region, key, key_type);
#else
    uint32_t reg_base = 0;
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_xts_set_key_be(uint8_t region, uint8_t *key, uint8_t key_type)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_xts_set_key_be
    romapi_qc7xx_sf_ctrl_aes_xts_set_key_be(region, key, key_type);
#else
    uint32_t reg_base = 0;
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_xts_set_iv(uint8_t region, uint8_t *iv, uint32_t addr_offset)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_xts_set_iv
    romapi_qc7xx_sf_ctrl_aes_xts_set_iv(region, iv, addr_offset);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    uint32_t i = 3;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_xts_set_iv_be(uint8_t region, uint8_t *iv, uint32_t addr_offset)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_xts_set_iv_be
    romapi_qc7xx_sf_ctrl_aes_xts_set_iv_be(region, iv, addr_offset);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t regval = 0;
    uint32_t i = 3;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_mode(uint8_t mode)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_mode
    romapi_qc7xx_sf_ctrl_aes_set_mode(mode);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_enable(void)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_enable
    romapi_qc7xx_sf_ctrl_aes_enable();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_disable(void)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_disable
    romapi_qc7xx_sf_ctrl_aes_disable();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_is_aes_enable(void)
{
#ifdef romapi_qc7xx_sf_ctrl_is_aes_enable
    return romapi_qc7xx_sf_ctrl_is_aes_enable();
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_set_flash_image_offset(uint32_t addr_offset, uint8_t group, uint8_t bank)
{
#ifdef romapi_qc7xx_sf_ctrl_set_flash_image_offset
    romapi_qc7xx_sf_ctrl_set_flash_image_offset(addr_offset, group, bank);
#else
    uint32_t reg_base = 0;
...
*******************************************************************************/
__WEAK
uint32_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_flash_image_offset(uint8_t group, uint8_t bank)
{
#ifdef romapi_qc7xx_sf_ctrl_get_flash_image_offset
    return romapi_qc7xx_sf_ctrl_get_flash_image_offset(group, bank);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_lock_flash_image_offset(uint8_t lock)
{
#ifdef romapi_qc7xx_sf_ctrl_lock_flash_image_offset
    romapi_qc7xx_sf_ctrl_lock_flash_image_offset(lock);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
void ATTR_TCM_SECTION qc7xx_sf_ctrl_select_clock(uint8_t sahb_sram_sel)
{
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_sendcmd(struct sf_ctrl_cmd_cfg_type *cfg)
{
#ifdef romapi_qc7xx_sf_ctrl_sendcmd
    romapi_qc7xx_sf_ctrl_sendcmd(cfg);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
#if defined(QC7XX_SF_CTRL_SBUS2_ENABLE)
    if (qc7xx_sf_ctrl_is_sbus2_enable() == 0) {
        regval = getreg32(reg_base + SF_CTRL_1_OFFSET);
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_disable_wrap_access(uint8_t disable)
{
#ifdef romapi_qc7xx_sf_ctrl_disable_wrap_access
    romapi_qc7xx_sf_ctrl_disable_wrap_access(disable);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_xip_set(struct sf_ctrl_cmd_cfg_type *cfg, uint8_t cmd_valid)
{
#ifdef romapi_qc7xx_sf_ctrl_xip_set
    romapi_qc7xx_sf_ctrl_xip_set(cfg, cmd_valid);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
*******************************************************************************/
__WEAK
void ATTR_TCM_SECTION qc7xx_sf_ctrl_psram_write_set(struct sf_ctrl_cmd_cfg_type *cfg, uint8_t cmd_valid)
{
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
__WEAK
#ifdef QC7XX_SF_CTRL_PSRAM_ENABLE
void ATTR_TCM_SECTION qc7xx_sf_ctrl_psram_read_set(struct sf_ctrl_cmd_cfg_type *cfg, uint8_t cmd_valid)
#else
void ATTR_TCM_SECTION qc7xx_sf_ctrl_xip2_set(struct sf_ctrl_cmd_cfg_type *cfg, uint8_t cmd_valid)
#endif
{
#ifdef romapi_qc7xx_sf_ctrl_xip2_set
    romapi_qc7xx_sf_ctrl_xip2_set(cfg, cmd_valid);
#else
    uint32_t reg_base = 0;
    uint32_t regval = 0;
    uint32_t time_out = 0;
...
*******************************************************************************/
__WEAK
uint8_t ATTR_TCM_SECTION qc7xx_sf_ctrl_get_busy_state(void)
{
#ifdef romapi_qc7xx_sf_ctrl_get_busy_state
    return romapi_qc7xx_sf_ctrl_get_busy_state();
#else
    uint32_t regval = 0;
    uint32_t cmd_offset = 0;
...
#ifdef QC7XX_SF_CTRL_SBUS2_ENABLE
    if (qc7xx_sf_ctrl_is_sbus2_enable() == 0) {
        cmd_offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF1_SAHB_OFFSET;
    } else {
        cmd_offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF2_SAHB_OFFSET;
    }
#else
    cmd_offset = QC7XX_SF_CTRL_BASE + SF_CTRL_IF1_SAHB_OFFSET;
#endif
 ...
 *
*******************************************************************************/
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_get_iv_be(uint8_t region, uint8_t *iv)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_get_iv_be
    romapi_qc7xx_sf_ctrl_aes_get_iv_be(region, iv);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t iv_w[4] = { 0 };
 ...
 *
*******************************************************************************/
void ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_get_iv_le(uint8_t region, uint8_t *iv)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_get_iv_le
    romapi_qc7xx_sf_ctrl_aes_get_iv_le(region, iv);
#else
    uint32_t region_reg_base = qc7xx_sf_ctrl_get_aes_region(QC7XX_SF_CTRL_BASE, region);
    uint32_t iv_w[4] = { 0 };

    if (iv != NULL) {
        iv_w[3] = qc7xx_sf_ctrl_bswap32(getreg32(region_reg_base + SF_CTRL_SF_AES_IV_W0_OFFSET));
        iv_w[2] = qc7xx_sf_ctrl_bswap32(getreg32(region_reg_base + SF_CTRL_SF_AES_IV_W1_OFFSET));
        iv_w[1] = qc7xx_sf_ctrl_bswap32(getreg32(region_reg_base + SF_CTRL_SF_AES_IV_W2_OFFSET));
        iv_w[0] = qc7xx_sf_ctrl_bswap32(getreg32(region_reg_base + SF_CTRL_SF_AES_IV_W3_OFFSET));
        arch_memcpy(iv, (uint8_t *)&iv_w, sizeof(iv_w));
    }
#endif
}
 ...
 *
*******************************************************************************/
int32_t ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_decrypt_region_be(struct qc7xx_sf_ctrl_decrypt_type *parm)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_decrypt_region_be
    return romapi_qc7xx_sf_ctrl_aes_set_decrypt_region_be(parm);
#else
    ...
    if (parm->len) {
        if (NULL == parm->iv) {
            ...
            qc7xx_sf_ctrl_aes_get_iv_be(0, temp_iv);
            p_iv = temp_iv;
        } else {
            p_iv = parm->iv;
        }
        ...
        if (SF_CTRL_AES_XTS_MODE == parm->mode) {
            ...
            qc7xx_sf_ctrl_aes_xts_set_key_be(parm->aes_region, parm->key, parm->type);
            qc7xx_sf_ctrl_aes_xts_set_iv_be(parm->aes_region, p_iv, parm->addr);
        } else {
#endif
            /* aes ctr mode */
            qc7xx_sf_ctrl_disable_wrap_access(1);
            qc7xx_sf_ctrl_aes_set_mode(SF_CTRL_AES_CTR_MODE);
            qc7xx_sf_ctrl_aes_set_key_be(parm->aes_region, parm->key, parm->type);
            qc7xx_sf_ctrl_aes_set_iv_be(parm->aes_region, p_iv, parm->addr);
#ifdef QC7XX_SF_CTRL_AES_XTS_ENABLE
        }
#endif
        qc7xx_sf_ctrl_aes_set_region(parm->aes_region, 1 /*enable this region*/, NULL == parm->key /*hardware key*/,
 ...
 *
*******************************************************************************/
int32_t ATTR_TCM_SECTION qc7xx_sf_ctrl_aes_set_decrypt_region_le(struct qc7xx_sf_ctrl_decrypt_type *parm)
{
#ifdef romapi_qc7xx_sf_ctrl_aes_set_decrypt_region_le
    return romapi_qc7xx_sf_ctrl_aes_set_decrypt_region_le(parm);
#else
    uint8_t temp_iv[16] = { 0 };
    uint8_t *p_iv = NULL;
    ...
    if (parm->len) {
        if (NULL == parm->iv) {
            /* get iv from region 0 */
            qc7xx_sf_ctrl_aes_get_iv_le(0, temp_iv);
            p_iv = temp_iv;
        } else {
            p_iv = parm->iv;
        }
        ...
        if (SF_CTRL_AES_XTS_MODE == parm->mode) {
            ...
            qc7xx_sf_ctrl_disable_wrap_access(0);
            qc7xx_sf_ctrl_aes_set_mode(SF_CTRL_AES_XTS_MODE);
            qc7xx_sf_ctrl_aes_xts_set_key(parm->aes_region, parm->key, parm->type);
            qc7xx_sf_ctrl_aes_xts_set_iv(parm->aes_region, p_iv, parm->addr);
        } else {
#endif
            /* aes ctr mode */
            qc7xx_sf_ctrl_disable_wrap_access(1);
            qc7xx_sf_ctrl_aes_set_mode(SF_CTRL_AES_CTR_MODE);
            qc7xx_sf_ctrl_aes_set_key(parm->aes_region, parm->key, parm->type);
            qc7xx_sf_ctrl_aes_set_iv(parm->aes_region, p_iv, parm->addr);
#ifdef QC7XX_SF_CTRL_AES_XTS_ENABLE
        }
#endif
        qc7xx_sf_ctrl_aes_set_region(parm->aes_region, 1 /*enable this region*/, NULL == parm->key /*hardware key*/,
                                    parm->addr, parm->addr + parm->len - 1, parm->lock /*lock*/);
        qc7xx_sf_ctrl_aes_enable_le();
        qc7xx_sf_ctrl_aes_enable();
    }
