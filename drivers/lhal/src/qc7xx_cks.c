...
{
#ifdef romapi_qc7xx_cks_reset
    romapi_qc7xx_cks_reset(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
{
#ifdef romapi_qc7xx_cks_set_endian
    romapi_qc7xx_cks_set_endian(dev, endian);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qc7xx_cks_compute
    return romapi_qc7xx_cks_compute(dev, data, length);
#else
    uint32_t reg_base;
    uint32_t i;
