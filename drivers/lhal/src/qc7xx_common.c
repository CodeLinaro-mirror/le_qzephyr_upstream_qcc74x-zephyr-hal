...
#define QC7XX_COMMON_UINT32_BIT_LEN  (32)
#define QC7XX_COMMON_UINT64_BIT_LEN  (64)
#define QC7XX_COMMON_UINT96_BIT_LEN  (96)
#define QC7XX_COMMON_UINT128_BIT_LEN (128)
...
#define QC7XX_BOOT2_IMG_OFFSET 8 * 1024
...
__WEAK __attribute__((optimize("O0"))) void *ATTR_TCM_SECTION arch_memset(void *s, uint8_t c, uint32_t n)
{
...
#if defined(QCC74X)
bool qc7xx_check_cache_addr(const void *addr)
{
...
void *qc7xx_get_no_cache_addr(const void *addr)
{
    uintptr_t a = (uintptr_t)addr;
...
bool qc7xx_check_cache_addr_aligned(uintptr_t addr)
{
    if (addr & (QC7XX_CACHE_LINE_SIZE - 1)) {
        return false;
    } else {
        return true;
    }
...
uint16_t qc7xx_soft_crc16(void *in, uint32_t len)
{
#ifdef romapi_qc7xx_soft_crc16
    return romapi_qc7xx_soft_crc16(in, len);
#else
...
* Use:     WinRAR,ect.
*****************************************************************************/
uint32_t ATTR_TCM_SECTION qc7xx_soft_crc32_ex(uint32_t initial, void *in, uint32_t len)
{
#ifdef romapi_qc7xx_soft_crc32_ex
    return romapi_qc7xx_soft_crc32_ex(initial, in, len);
#else
    uint8_t i;
...
__WEAK uint32_t ATTR_TCM_SECTION qc7xx_soft_crc32(void *in, uint32_t len)
{
#ifdef romapi_qc7xx_soft_crc32
    return romapi_qc7xx_soft_crc32(in, len);
#else
    return qc7xx_soft_crc32_ex(0, in, len);
#endif
}
...
__WEAK int32_t qc7xx_get_app_version_from_efuse(uint8_t *version)
{
    ...
    return romapi_qc7xx_get_app_version_from_efuse(version);
#else
#if defined(QCC74X) || defined(QCC75X)
    ...
    qc7xx_ef_ctrl_read_direct(NULL, ANTI_ROLLBACK_ENABLE_OFFSET, &tmpVal, 1, 1);
...
#endif
    /* read efuse value */
    qc7xx_ef_ctrl_read_direct(NULL, ANTI_ROLLBACK_APP_VERSION_OFFSET, value, sizeof(value) / 4, 1);
#if defined(QCC74X)
    ...
    if (version_high_high) {
        *version = QC7XX_COMMON_UINT128_BIT_LEN - __builtin_clz(version_high_high);
        return 0;
    }

    /* version_real[95:64] case */
    if (version_high) {
        *version = QC7XX_COMMON_UINT96_BIT_LEN - __builtin_clz(version_high);
        return 0;
    }

    /* version_real[63:32] case */
    if (version_low) {
        *version = QC7XX_COMMON_UINT64_BIT_LEN - __builtin_clz(version_low);
        return 0;
    }

    /* version_real[31:0] case */
    if (version_low_low) {
        *version = QC7XX_COMMON_UINT32_BIT_LEN - __builtin_clz(version_low_low);
        return 0;
    }
#endif
    *version = 0;
...
__WEAK int32_t qc7xx_set_app_version_to_efuse(uint8_t version)
{
#ifdef romapi_qc7xx_set_app_version_to_efuse
    return romapi_qc7xx_set_app_version_to_efuse(version);
#else
#if defined(QCC74X) || defined(QCC75X)
    uint8_t version_old;
    uint32_t tmpVal;
    ...
    qc7xx_ef_ctrl_write_direct(NULL, ANTI_ROLLBACK_ENABLE_OFFSET, &tmpVal, 0x1, 1);

    if (qc7xx_get_app_version_from_efuse(&version_old) != 0) {
        return 1;
    }
    ...
    if (version <= QC7XX_COMMON_UINT32_BIT_LEN) {
        version_low_low = (1 << (version - 1));
    } else if (version <= QC7XX_COMMON_UINT64_BIT_LEN) {
        version_low = (1 << (version - QC7XX_COMMON_UINT32_BIT_LEN - 1));
    } else if (version <= QC7XX_COMMON_UINT96_BIT_LEN) {
        version_high = (1 << (version - QC7XX_COMMON_UINT64_BIT_LEN - 1));
    } else if (version <= QC7XX_COMMON_UINT128_BIT_LEN) {
        version_high_high = (1 << (version - QC7XX_COMMON_UINT96_BIT_LEN - 1));
    }
#if defined(QCC74X)
...
#endif
    /* write efuse value */
    qc7xx_ef_ctrl_write_direct(NULL, ANTI_ROLLBACK_APP_VERSION_OFFSET, value, sizeof(value) / 4, 1);
#endif
    return 0;
#else
    return 1;
...
__WEAK int32_t qc7xx_get_boot2_version_from_efuse(uint8_t *version)
{
#ifdef romapi_qc7xx_get_boot2_version_from_efuse
    return romapi_qc7xx_get_boot2_version_from_efuse(version);
#else
#if defined(QCC74X) || defined(QCC75X)
    uint32_t otp_ef_boot2_anti_rollback_en = 0;
    uint32_t tmpVal;
    ...
    /* read efuse value */
    qc7xx_ef_ctrl_read_direct(NULL, ANTI_ROLLBACK_BOOT2_VERSION_OFFSET, value, sizeof(value) / 4, 1);
#if defined(QCC74X)
    /* get real version from efuse */
    ...
    /* version_real[63:32] case */
    if (version_high) {
        *version = QC7XX_COMMON_UINT64_BIT_LEN - __builtin_clz(version_high);
        return 0;
    }

    /* version_real[31:0] case */
    if (version_low) {
        *version = QC7XX_COMMON_UINT32_BIT_LEN - __builtin_clz(version_low);
        return 0;
    }
...
__WEAK int32_t qc7xx_set_boot2_version_to_efuse(uint8_t version)
{
#ifdef romapi_qc7xx_set_boot2_version_to_efuse
    return romapi_qc7xx_set_boot2_version_to_efuse(version);
#else
#if defined(QCC74X) || defined(QCC75X)
    uint8_t version_old;
    uint32_t tmpVal;
    ...
    if (qc7xx_get_boot2_version_from_efuse(&version_old) != 0) {
        return 1;
    }
    ...
    if (version <= QC7XX_COMMON_UINT32_BIT_LEN) {
        ...
        version_high = (1 << (version - QC7XX_COMMON_UINT32_BIT_LEN - 1));
    }
#if defined(QCC74X)
...
#endif
    /* write efuse value */
    qc7xx_ef_ctrl_write_direct(NULL, ANTI_ROLLBACK_BOOT2_VERSION_OFFSET, value, sizeof(value) / 4, 1);
...
__WEAK int32_t qc7xx_get_boot2_info_from_flash(qc7xx_verinf_t *version)
{
    uint32_t addr = 0xC08;
    qc7xx_flash_read(addr + QC7XX_BOOT2_IMG_OFFSET, (uint8_t *)version, sizeof(qc7xx_verinf_t));
