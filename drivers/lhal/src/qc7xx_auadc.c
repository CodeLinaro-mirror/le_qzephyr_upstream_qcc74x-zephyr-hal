...
#include "qc7xx_auadc.h"
#include "hardware/auadc_reg.h"

int qc7xx_auadc_init(struct qc7xx_device_s *dev, const struct qc7xx_auadc_init_config_s *config)
{
...
#ifdef romapi_qc7xx_auadc_init
    return romapi_qc7xx_auadc_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
...
#if (AUADC_ANALOG_ADC_SUPPORT)
int qc7xx_auadc_adc_init(struct qc7xx_device_s *dev, const struct qc7xx_auadc_adc_init_config_s *adc_analog_cfg)
{
    LHAL_PARAM_ASSERT(dev);
...
#ifdef romapi_qc7xx_auadc_adc_init
    return romapi_qc7xx_auadc_adc_init(dev, adc_analog_cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_auadc_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_auadc_link_rxdma
    return romapi_qc7xx_auadc_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_auadc_int_mask(struct qc7xx_device_s *dev, uint32_t int_sts)
{
#ifdef romapi_qc7xx_auadc_int_mask
    return romapi_qc7xx_auadc_int_mask(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_auadc_int_unmask(struct qc7xx_device_s *dev, uint32_t int_sts)
{
#ifdef romapi_qc7xx_auadc_int_unmask
    return romapi_qc7xx_auadc_int_unmask(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_auadc_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_auadc_get_intstatus
    return romapi_qc7xx_auadc_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
    int32_t int_sts;
...
int qc7xx_auadc_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_auadc_feature_control
    return romapi_qc7xx_auadc_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
