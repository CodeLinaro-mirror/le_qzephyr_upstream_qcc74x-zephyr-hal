...
__UNUSED static void qc7xx_mjpeg_set_yuv422_interleave_order(struct qc7xx_device_s *dev, uint8_t y0, uint8_t u0, uint8_t y1, uint8_t v0)
{
    uint32_t regval;
    uint32_t reg_base;
...
__UNUSED static void qc7xx_mjpeg_set_framesize(struct qc7xx_device_s *dev, uint16_t x, uint16_t y)
{
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_init(struct qc7xx_device_s *dev, const struct qc7xx_mjpeg_config_s *config)
{
...
#ifdef romapi_qc7xx_mjpeg_init
    romapi_qc7xx_mjpeg_init(dev, config);
#else
    uint32_t regval;
    uint32_t reg_base;
    uint16_t blocks;
    ...
    switch (config->format) {
        case MJPEG_FORMAT_YUV422_YUYV:
            ...
            qc7xx_mjpeg_set_yuv422_interleave_order(dev, 0, 1, 2, 3);
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 15) >> 4, (config->resolution_y + 7) >> 3);
            break;
        case MJPEG_FORMAT_YUV422_YVYU:
            regval |= (3 << MJPEG_REG_YUV_MODE_SHIFT);
            qc7xx_mjpeg_set_yuv422_interleave_order(dev, 0, 3, 2, 1);
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 15) >> 4, (config->resolution_y + 7) >> 3);
            break;
        case MJPEG_FORMAT_YUV422_UYVY:
            regval |= (3 << MJPEG_REG_YUV_MODE_SHIFT);
            qc7xx_mjpeg_set_yuv422_interleave_order(dev, 1, 0, 3, 2);
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 15) >> 4, (config->resolution_y + 7) >> 3);
            break;
        case MJPEG_FORMAT_YUV422_VYUY:
            regval |= (3 << MJPEG_REG_YUV_MODE_SHIFT);
            qc7xx_mjpeg_set_yuv422_interleave_order(dev, 1, 2, 3, 0);
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 15) >> 4, (config->resolution_y + 7) >> 3);
            break;
        ...
        case MJPEG_FORMAT_YUV420SP_NV12:
            ...
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 15) >> 4, (config->resolution_y + 15) >> 4);
            break;
        ...
        case MJPEG_FORMAT_GRAY:
            ...
            qc7xx_mjpeg_set_framesize(dev, (config->resolution_x + 7) >> 3, (config->resolution_y + 7) >> 3);
            break;
    ...
    if (config->input_yy_table) {
        qc7xx_mjpeg_calculate_quantize_table(config->quality, config->input_yy_table, tmp_table_y);
    } else {
        qc7xx_mjpeg_calculate_quantize_table(config->quality, (uint16_t *)q_table_50_y, tmp_table_y);
    }
    if (config->input_uv_table) {
        qc7xx_mjpeg_calculate_quantize_table(config->quality, config->input_uv_table, tmp_table_uv);
    } else {
        qc7xx_mjpeg_calculate_quantize_table(config->quality, (uint16_t *)q_table_50_uv, tmp_table_uv);
    }

    qc7xx_mjpeg_fill_quantize_table(dev, tmp_table_y, tmp_table_uv);
#endif
}

void qc7xx_mjpeg_start(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_start
    romapi_qc7xx_mjpeg_start(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_stop(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_stop
    romapi_qc7xx_mjpeg_stop(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_sw_run(struct qc7xx_device_s *dev, uint8_t frame_count)
{
#ifdef romapi_qc7xx_mjpeg_sw_run
    romapi_qc7xx_mjpeg_sw_run(dev, frame_count);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_kick_run(struct qc7xx_device_s *dev, uint16_t kick_count)
{
#ifdef romapi_qc7xx_mjpeg_kick_run
    romapi_qc7xx_mjpeg_kick_run(dev, kick_count);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_kick_stop(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_kick_stop
    romapi_qc7xx_mjpeg_kick_stop(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_kick(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_kick
    romapi_qc7xx_mjpeg_kick(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_tcint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_mjpeg_tcint_mask
    romapi_qc7xx_mjpeg_tcint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_swapint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_mjpeg_swapint_mask
    romapi_qc7xx_mjpeg_swapint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_mjpeg_errint_mask
    romapi_qc7xx_mjpeg_errint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base;
...
uint32_t qc7xx_mjpeg_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_get_intstatus
    return romapi_qc7xx_mjpeg_get_intstatus(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
#ifdef romapi_qc7xx_mjpeg_int_clear
    romapi_qc7xx_mjpeg_int_clear(dev, int_clear);
#else
...
uint8_t qc7xx_mjpeg_get_frame_count(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_get_frame_count
    return romapi_qc7xx_mjpeg_get_frame_count(dev);
#else
    uint32_t reg_base;
...
void qc7xx_mjpeg_pop_one_frame(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_pop_one_frame
    romapi_qc7xx_mjpeg_pop_one_frame(dev);
#else
    uint32_t reg_base;
...
void qc7xx_mjpeg_pop_swap_block(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_pop_swap_block
    romapi_qc7xx_mjpeg_pop_swap_block(dev);
#else
    uint32_t reg_base;
...
uint32_t qc7xx_mjpeg_get_swap_bit_count(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_mjpeg_get_swap_bit_count
    return romapi_qc7xx_mjpeg_get_swap_bit_count(dev);
#else
    uint32_t reg_base;
...
uint32_t qc7xx_mjpeg_get_frame_info(struct qc7xx_device_s *dev, uint8_t **pic)
{
#ifdef romapi_qc7xx_mjpeg_get_frame_info
    return romapi_qc7xx_mjpeg_get_frame_info(dev, pic);
#else
    uint32_t bytes;
    uint32_t reg_base;
...
uint8_t qc7xx_mjpeg_get_swap_block_info(struct qc7xx_device_s *dev, uint8_t *idx)
{
#ifdef romapi_qc7xx_mjpeg_get_swap_block_info
    return romapi_qc7xx_mjpeg_get_swap_block_info(dev, idx);
#else
    uint32_t regval;
    uint32_t reg_base;
...
uint8_t qc7xx_mjpeg_swap_is_block_full(struct qc7xx_device_s *dev, uint8_t idx)
{
#ifdef romapi_qc7xx_mjpeg_swap_is_block_full
    return romapi_qc7xx_mjpeg_swap_is_block_full(dev, idx);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_calculate_quantize_table(uint8_t quality, uint16_t *input_table, uint16_t *output_table)
{
#ifdef romapi_qc7xx_mjpeg_calculate_quantize_table
    romapi_qc7xx_mjpeg_calculate_quantize_table(quality, input_table, output_table);
#else
...
void qc7xx_mjpeg_fill_quantize_table(struct qc7xx_device_s *dev, uint16_t *input_yy, uint16_t *input_uv)
{
#ifdef romapi_qc7xx_mjpeg_fill_quantize_table
    romapi_qc7xx_mjpeg_fill_quantize_table(dev, input_yy, input_uv);
#else
...
void qc7xx_mjpeg_fill_jpeg_header_tail(struct qc7xx_device_s *dev, uint8_t *header, uint32_t header_len)
{
#ifdef romapi_qc7xx_mjpeg_fill_jpeg_header_tail
    romapi_qc7xx_mjpeg_fill_jpeg_header_tail(dev, header, header_len);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_set_yuv420sp_cam_input(struct qc7xx_device_s *dev, uint8_t yy, uint8_t uv)
{
#ifdef romapi_qc7xx_mjpeg_set_yuv420sp_cam_input
    romapi_qc7xx_mjpeg_set_yuv420sp_cam_input(dev, yy, uv);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_mjpeg_update_input_output_buff(struct qc7xx_device_s *dev, void *input_buf0, void *input_buf1, void *output_buff, size_t output_buff_size)
{
#ifdef romapi_qc7xx_mjpeg_update_input_output_buff
    romapi_qc7xx_mjpeg_update_input_output_buff(dev, input_buf0, input_buf1, output_buff, output_buff_size);
#else
    uint32_t reg_base;
...
int qc7xx_mjpeg_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_mjpeg_feature_control
    return romapi_qc7xx_mjpeg_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    __UNUSED uint32_t regval;
    uint32_t reg_base;
