...
int qc7xx_pec_pwm_init(struct qc7xx_device_s *dev, struct qc7xx_pec_pwm_s *pwm)
{
    ...
    ret = qc7xx_pec_memory_size_check(dev, pwm->mem, 5, &mem);
    if (ret != PEC_STS_OK) {
        return ret;
    }
    ...
    qc7xx_pec_mem_write(dev, mem + 0, pec_instr_MOVREG(PEC_INSTR_MOV_DST_R3, PEC_INSTR_MOV_SRC_R0, PEC_INSTR_MOV_OP_NONE, pec_instr_side(3, 0, 2))); /* set pin high */
    qc7xx_pec_mem_write(dev, mem + 1, pec_instr_JRNZ(mem + 1, PEC_R3, PEC_INSTR_CMP_DEC, pec_instr_side(0, 0, 2))); /* high level time */
    qc7xx_pec_mem_write(dev, mem + 2, pec_instr_MOVREG(PEC_INSTR_MOV_DST_R3, PEC_INSTR_MOV_SRC_R1, PEC_INSTR_MOV_OP_NONE, pec_instr_side(2, 0, 2))); /* set pin low */
    qc7xx_pec_mem_write(dev, mem + 3, pec_instr_JRNZ(mem + 3, PEC_R3, PEC_INSTR_CMP_DEC, pec_instr_side(0, 0, 2))); /* low level time */
    qc7xx_pec_mem_write(dev, mem + 4, pec_instr_JBZ(mem + 4, PEC_R2, 0, PEC_INSTR_CMP_NONE, pec_instr_side(0, 0, 2))); /* judge enable bit */
...
void qc7xx_pec_pwm_start(struct qc7xx_device_s *dev, uint32_t high_cnt, uint32_t low_cnt)
{
    ...
    qc7xx_pec_sm_reg_write(dev, PEC_R0, high_cnt);
    qc7xx_pec_sm_reg_write(dev, PEC_R1, low_cnt);
    qc7xx_pec_sm_reg_set_bit(dev, PEC_R2, 0);
}
...
{
    qc7xx_pec_sm_reg_clr_bit(dev, PEC_R2, 0);
}
