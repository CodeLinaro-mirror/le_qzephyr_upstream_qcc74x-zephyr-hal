...
#include "qc7xx_emac_v2.h"
#include "qc7xx_clock.h"
#include "qc7xx_l1c.h"

#define DEFAULT_LOOP_VARIABLE  1000
#define DEFAULT_DELAY_VARIABLE 10
#define emac_v2_delay_us       qc7xx_mtimer_delay_us
...
struct qc7xx_emac_v2_handle_s {
    ...
    struct qc7xx_emac_v2_dma_desc_s *rx_dma_desc;
    uint32_t rx_bytes;
    uint32_t rx_packets;
    uint32_t rx_errors;
...
static struct qc7xx_emac_v2_handle_s eth_handle;
static struct qc7xx_emac_v2_handle_s *thiz = NULL;

static ATTR_NOCACHE_NOINIT_RAM_SECTION __attribute__((aligned(4))) struct qc7xx_emac_v2_dma_desc_s tx_dma_desc[EMAC_TX_BD_NUM_MAX];
static ATTR_NOCACHE_NOINIT_RAM_SECTION __attribute__((aligned(4))) struct qc7xx_emac_v2_dma_desc_s rx_dma_desc[EMAC_RX_BD_NUM_MAX];

void qc7xx_emac_v2_isr(int irq, void *arg);

/* isr event callback */
static void *emac_irq_arg = NULL;
static qc7xx_emac_irq_cb_t emac_irq_event_cb = NULL;
...
/**
 * @brief qc7xx emac_v2 dma description list init
 *
 * @param dev
 * @param handle
 * @param tx_dma_desc
 ...
 * 
 */
static int qc7xx_emac_v2_dma_desc_list_init(struct qc7xx_device_s *dev, struct qc7xx_emac_v2_handle_s *handle,
                                           struct qc7xx_emac_v2_dma_desc_s *tx_dma_desc,
                                           uint8_t *tx_buff,
                                           ...
                                           uint32_t rx_dma_desc_cnt)
{
    uint32_t i = 0;
    uint32_t reg_base;
    struct qc7xx_emac_v2_dma_desc_s *dma_desc;
    ...
    for (i = 0; i < tx_dma_desc_cnt; i++) {
        ...
        qc7xx_emac_v2_tx_dma_desc_init(dma_desc, i == tx_dma_desc_cnt - 1);
    ...
    for (i = 0; i < rx_dma_desc_cnt; i++) {
        ...
        qc7xx_emac_v2_rx_dma_desc_init(dma_desc, i == rx_dma_desc_cnt - 1);
        if (NULL != rx_buff) {
                               ...
                               struct qc7xx_emac_v2_dma_desc_s *rx_dma_desc, uint8_t *rx_buff, uint32_t rx_dma_desc_cnt)
{
    ...
    return qc7xx_emac_v2_dma_desc_list_init(dev, thiz, tx_dma_desc, tx_buff, tx_dma_desc_cnt,
                             ...
                             uint32_t *ext_status, uint32_t *time_stamp_high, uint32_t *time_stamp_low)
{
    ...
    struct qc7xx_emac_v2_dma_desc_s *txdesc = thiz->tx_dma_desc + thiz->tx_index_emac_v2;

    EMAC_V2_DRV_DBG("Check txdesc sts=%08x,len=%08x\r\n", txdesc->status, txdesc->length);

    if (qc7xx_emac_v2_is_desc_owned_by_dma(txdesc)) {
        return -1;
    }

    if (qc7xx_emac_v2_is_desc_empty(txdesc)) {
        return -1;
    }
    ...
    qc7xx_emac_v2_tx_dma_desc_init(txdesc, qc7xx_emac_v2_is_last_tx_desc(txdesc));
    thiz->tx_index_emac_v2 = qc7xx_emac_v2_is_last_tx_desc(txdesc) ? 0 : txover + 1;
                             ...
                             uint32_t buffer2, uint32_t length2, uint32_t data2, uint32_t offload_needed)
{
    ...
    struct qc7xx_emac_v2_dma_desc_s *txdesc = thiz->tx_dma_desc + thiz->tx_index_cpu;
    ...
    if (offload_needed) {
        ...
        */
        qc7xx_emac_v2_tx_checksum_offload_ipv4hdr(txdesc);
        qc7xx_emac_v2_tx_checksum_offload_tcponly(txdesc);
    }
#endif

    txdesc->status |= EMAC_V2_DESC_OWN_BY_DMA;

    thiz->tx_index_cpu = qc7xx_emac_v2_is_last_tx_desc(txdesc) ? 0 : txnext + 1;
                             ...
                             uint32_t buffer2, uint32_t length2, uint32_t data2)
{
    ...
    struct qc7xx_emac_v2_dma_desc_s *rxdesc = thiz->rx_dma_desc + thiz->rx_index_cpu;

#if 0
    if (!qc7xx_emac_v2_is_desc_empty(rxdesc)) {
        EMAC_V2_DRV_DBG("rxdesc is not empty, rxnext=%d\r\n", rxnext);
        return -1;
    }
#endif

    qc7xx_emac_v2_rx_dma_desc_init(rxdesc, qc7xx_emac_v2_is_last_rx_desc(rxdesc));
    ...
    thiz->rx_index_cpu = qc7xx_emac_v2_is_last_rx_desc(rxdesc) ? 0 : rxnext + 1;
                             ...
                             uint32_t *ext_status, uint32_t *time_stamp_high, uint32_t *time_stamp_low)
{
    ...
    struct qc7xx_emac_v2_dma_desc_s *rxdesc = thiz->rx_dma_desc + thiz->rx_index_emac_v2;

    if (qc7xx_emac_v2_is_desc_owned_by_dma(rxdesc)) {
        return -1;
    }
    ...
    thiz->rx_index_emac_v2 = qc7xx_emac_v2_is_last_rx_desc(rxdesc) ? 0 : rxnext + 1;
#if 1
...
int qc7xx_emac_v2_take_desc_ownership_rx(struct qc7xx_device_s *dev)
{
    ...
    for (i = 0; i < thiz->rx_index_cnt; i++) {
        qc7xx_emac_v2_take_desc_ownership((thiz->rx_dma_desc + i));
    }
...
int qc7xx_emac_v2_take_desc_ownership_tx(struct qc7xx_device_s *dev)
{
    ...
    for (i = 0; i < thiz->tx_index_cnt; i++) {
        qc7xx_emac_v2_take_desc_ownership((thiz->tx_dma_desc + i));
    }
 ...
 * 
 */
int qc7xx_emac_init(struct qc7xx_device_s *dev, const struct qc7xx_emac_config_s *config)
{
    qc7xx_emac_v2_reset(dev);

    /* set mac address */
    qc7xx_emac_v2_mac_addr_set(dev, config->mac_addr);

    /* set mdio clock */
    qc7xx_emac_v2_mdc_clk_div_set(dev, config->md_clk_div);

    /* set emac_v2 core */
    qc7xx_emac_v2_wd_enable(dev);
    qc7xx_emac_v2_jabber_enable(dev);
    qc7xx_emac_v2_frame_burst_enable(dev);
    qc7xx_emac_v2_jumbo_frame_disable(dev);
    qc7xx_emac_v2_rx_own_enable(dev);
    qc7xx_emac_v2_loopback_disable(dev);

    qc7xx_emac_v2_retry_enable(dev);
    qc7xx_emac_v2_pad_crc_strip_disable(dev);
    qc7xx_emac_v2_back_off_limit_set(dev, EMAC_V2_PASS_CONTROL0);
    qc7xx_emac_v2_deferral_check_disable(dev);

    if (config->speed == 1000) {
        qc7xx_emac_v2_gmii_select(dev);
    } else {
        qc7xx_emac_v2_mii_select(dev);
        if (config->speed == 100) {
            qc7xx_emac_v2_mii_speed_set(dev, 100);
        } else if (config->speed == 10) {
            qc7xx_emac_v2_mii_speed_set(dev, 10);
        }
    }

    /* Frame Filter Configuration */
    qc7xx_emac_v2_frame_filter_enable(dev);
    qc7xx_emac_v2_pass_control_set(dev, EMAC_V2_PASS_CONTROL0);
    qc7xx_emac_v2_broadcast_enable(dev);
    qc7xx_emac_v2_src_addr_filter_disable(dev);
    qc7xx_emac_v2_multicast_disable(dev);
    qc7xx_emac_v2_dst_addr_filter_normal(dev);
    qc7xx_emac_v2_multicast_hash_filter_disable(dev);
    qc7xx_emac_v2_promisc_disable(dev);
    qc7xx_emac_v2_unicast_hash_filter_disable(dev);
    /* Flow Control Configuration */
    qc7xx_emac_v2_unicast_pause_frame_detect_disable(dev);

    if (config->duplex_mode == EAMC_FULLDUPLEX) {
        qc7xx_emac_v2_full_duplex_enable(dev);
        qc7xx_emac_v2_rx_flow_control_enable(dev);
        qc7xx_emac_v2_tx_flow_control_enable(dev);
    } else {
        /* for Half Duplex configuration */
        qc7xx_emac_v2_half_duplex_enable(dev);
        qc7xx_emac_v2_rx_flow_control_disable(dev);
        qc7xx_emac_v2_tx_flow_control_disable(dev);
...
#if 1
    qc7xx_emac_v2_dma_desc_init(dev, tx_dma_desc, NULL, EMAC_TX_BD_NUM_MAX,
                               rx_dma_desc, NULL, EMAC_RX_BD_NUM_MAX);
#else
    qc7xx_emac_v2_dma_desc_init(dev, tx_dma_desc, (uint8_t *)ethTxBuff, EMAC_TX_BD_NUM_MAX,
                               rx_dma_desc, NULL, EMAC_RX_BD_NUM_MAX);
#endif
    qc7xx_emac_v2_start_pre(dev);
#ifndef NOT_USE_QC7XX_LHAL_IRQ_ATTACH
    qc7xx_irq_attach(dev->irq_num, qc7xx_emac_v2_isr, dev);
    qc7xx_irq_enable(dev->irq_num);
#endif
    return 0;
}

/**
 * @brief qc7xx emac_v2 deinit
 *
 * @param dev
 *
 */
void qc7xx_emac_v2_deinit(struct qc7xx_device_s *dev)
{
}
...
{
    qc7xx_emac_v2_dma_bus_init(dev);
    qc7xx_emac_v2_dma_ctrl_init(dev);

    /* enable rx ipc offload */
    //qc7xx_emac_v2_rx_chksum_offload_enable(dev);
    //qc7xx_emac_v2_rx_tcpip_chksum_drop_disable(dev);

    qc7xx_emac_v2_promisc_enable(dev);
    //qc7xx_emac_v2_tx_enable(dev);
    //qc7xx_emac_v2_rx_enable(dev);
    qc7xx_emac_v2_enable_dma_rx(dev);
    qc7xx_emac_v2_enable_dma_tx(dev);
    qc7xx_emac_v2_enable_interrupt(dev, EMAC_V2_INT_ENABLE_CFG);
}
...
int qc7xx_emac_v2_handle_transmit_over(struct qc7xx_device_s *dev)
{
    ...
    do {
        desc_index = qc7xx_emac_v2_get_tx_qptr(dev, &status, &dma_addr1, &length1, &data1, &dma_addr2, &length2, &data2, &ext_status, &time_stamp_high, &time_stamp_low);
        if (desc_index < 0) {
            break;
        }
#ifdef EMAC_V2_DBG_ENABLE
        qc7xx_emac_v2_ts_read_timestamp_higher_val(dev, &time_stamp_higher);
#endif
        struct qc7xx_emac_trans_desc_s tx_desc = {
        ...
        if (desc_index >= 0 && dma_addr1 != 0) {
            ...
            EMAC_V2_DRV_DBG("timestamp_low =%08x,timestamp_high =%08x, timestamp_higher =%08x\r\n", time_stamp_low, time_stamp_high, time_stamp_higher);
#ifdef EMAC_V2_IPC_OFFLOAD
            if (qc7xx_emac_v2_is_tx_ipv4header_checksum_error(status)) {
                EMAC_V2_DRV_DBG("Harware Failed to Insert IPV4 Header Checksum\r\n");
            }
            if (qc7xx_emac_v2_is_tx_payload_checksum_error(status)) {
                EMAC_V2_DRV_DBG("Harware Failed to Insert Payload Checksum\r\n");
            }
#endif
            if (qc7xx_emac_v2_is_desc_valid(status)) {
                thiz->tx_bytes += length1;
                thiz->tx_packets++;
            } else {
                EMAC_V2_DRV_ERR("Error in Status %08x\r\n", status);
                thiz->tx_errors++;
                thiz->tx_aborted_errors += qc7xx_emac_v2_is_tx_aborted(status);
                thiz->tx_carrier_errors += qc7xx_emac_v2_is_tx_carrier_error(status);
        ...
        thiz->collisions += qc7xx_emac_v2_get_tx_collision_count(status);
    } while (1);
...
{
    int txnext = thiz->tx_index_cpu;
    struct qc7xx_emac_v2_dma_desc_s *txdesc = thiz->tx_dma_desc + txnext;
...
#if defined(QCC75X)
    qc7xx_l1c_dcache_clean_range((void *)txdesc->buffer1, length1);
#endif
#endif
    ...
    if ((qc7xx_emac_v2_get_interrupt_status(dev) & EMAC_V2_DMA_TX_STATE) == EMAC_V2_DMA_TX_SUSPENDED) {
        qc7xx_emac_v2_resume_dma_tx(dev);
    }
...
#endif
{
    ...
    *len = 0;
    ...
    if (qc7xx_emac_v2_is_ext_status(status)) {
    ...
    if (qc7xx_emac_v2_is_rx_desc_valid(status)) {
        *len = qc7xx_emac_v2_get_rx_desc_frame_length(status) - 4; //Not interested in Ethernet CRC bytes
#ifdef EMAC_V2_IPC_OFFLOAD
        ...
        */
        if (qc7xx_emac_v2_is_ext_status(status)) { // extended status present indicates that the RDES4 need to be probed
            EMAC_V2_DRV_ERR("Extended Status present\r\n");
            if (qc7xx_emac_v2_es_is_rx_checksum_bypassed(ext_status)) { // Hardware engine bypassed the checksum computation/checking
                EMAC_V2_DRV_ERR("(EXTSTS)Hardware bypassed checksum computation\r\n");
            }
            if ((status & EMAC_V2_DESC_RX_FRAME_ETHER) && qc7xx_emac_v2_es_is_ip_header_error(ext_status)) { // IP header (IPV4) checksum error
                EMAC_V2_DRV_ERR("(EXTSTS)Error in IP header error\r\n");
            }
            if ((status & EMAC_V2_DESC_RX_FRAME_ETHER) && qc7xx_emac_v2_es_is_ip_payload_error(ext_status)) { // IP payload checksum is in error (UDP/TCP/ICMP checksum error)
...
#if 0
            /* qc7xx_emac_v2_is_rx_checksum_error is not correct */
            if ((qc7xx_emac_v2_is_rx_checksum_error(dev, status) == EMAC_V2_RX_LEN_LT600)) {
                EMAC_V2_DRV_DBG("Err:EMAC_V2_RX_LEN_LT600\r\n");
            }
            if ((qc7xx_emac_v2_is_rx_checksum_error(dev, status) == EMAC_V2_RX_PAYLOAD_CHK_ERROR)) {
                EMAC_V2_DRV_DBG("Err:EMAC_V2_RX_PAYLOAD_CHK_ERROR\r\n");
            }
            if ((qc7xx_emac_v2_is_rx_checksum_error(dev, status) == EMAC_V2_RX_IPHDR_CHK_ERROR)) {
                EMAC_V2_DRV_DBG("Err:EMAC_V2_RX_IPHDR_CHK_ERROR\r\n");
            }
            if ((qc7xx_emac_v2_is_rx_checksum_error(dev, status) == EMAC_V2_RX_IPHDR_PAYLOAD_CHK_ERROR)) {
        ...
        thiz->rx_errors++;
        thiz->collisions += qc7xx_emac_v2_is_rx_frame_collision(status);
        thiz->rx_crc_errors += qc7xx_emac_v2_is_rx_crc(status);
        thiz->rx_frame_errors += qc7xx_emac_v2_is_frame_dribbling_errors(status);
        thiz->rx_length_errors += qc7xx_emac_v2_is_rx_frame_length_errors(status);
    }
...
/**
 * @brief qc7xx emac_v2 phy register read
 *
 * @param dev
 * @param phy_reg
 * @param phy_reg_val
 ...
 *
 */
int qc7xx_emac_md_read(struct qc7xx_device_s *dev, uint8_t phy_addr, uint16_t phy_reg, uint16_t *data)
{
    uint32_t reg_val;
...
/**
 * @brief qc7xx emac_v2 phy register write
 *
 * @param dev
 * @param phy_reg
 * @param phy_reg_val
 ...
 *
 */
int qc7xx_emac_md_write(struct qc7xx_device_s *dev, uint8_t phy_addr, uint16_t phy_reg, uint16_t phy_reg_val)
{
    uint32_t reg_val;
    uint32_t timeout = DEFAULT_LOOP_VARIABLE;
...
/**
 * @brief qc7xx emac_v2 feature control
 *
 * @param dev
 * @param cmd
 * @param arg
 ...
 *
 */
int qc7xx_emac_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
    int ret = 0;
    uint32_t reg_val;
    uint32_t reg_base;
...
/* isr callback attach */
int qc7xx_emac_irq_attach(struct qc7xx_device_s *dev, qc7xx_emac_irq_cb_t irq_event_cb, void *arg)
{
...
void qc7xx_emac_v2_isr(int irq, void *arg)
{
    ...
    uint32_t interrupt;
    struct qc7xx_device_s *emac_v2_dev = (struct qc7xx_device_s *)arg;
    uint32_t status;
    uint32_t ext_status;

    EMAC_V2_DRV_DBG("emac int come\r\n");

    dma_status_reg = qc7xx_emac_v2_get_interrupt_status(emac_v2_dev);
    if (dma_status_reg == 0) {
        return;
    }

    qc7xx_emac_v2_disable_interrupt_all(emac_v2_dev);

    /*Now lets handle the DMA interrupts*/
    interrupt = qc7xx_emac_v2_get_interrupt_type(emac_v2_dev);

    if (interrupt & EMAC_V2_DMA_INT_API_ERROR) {
        qc7xx_emac_v2_disable_dma_tx(emac_v2_dev);
        qc7xx_emac_v2_disable_dma_rx(emac_v2_dev);

        qc7xx_emac_v2_take_desc_ownership_tx(emac_v2_dev);
        qc7xx_emac_v2_take_desc_ownership_rx(emac_v2_dev);
        //qc7xx_emac_v2_init_tx_rx_desc_queue(emac_v2x);
        qc7xx_emac_v2_reset(emac_v2_dev);
    ...
    if (interrupt & EMAC_V2_DMA_INT_API_RX_NORMAL) {
        ...
        struct qc7xx_emac_trans_desc_s rx_desc;
        while (qc7xx_emac_v2_get_rx_qptr(emac_v2_dev, &status, (uint32_t *)&rx_desc.buff_addr, NULL, NULL,
                                        NULL, NULL, NULL, &ext_status, NULL, NULL) >= 0) {
            if (qc7xx_emac_v2_is_rx_desc_valid(status)) {
                rx_desc.data_len = qc7xx_emac_v2_get_rx_desc_frame_length(status) - 4;
    ...
    if (interrupt & EMAC_V2_DMA_INT_API_TX_STOPPED) {
        qc7xx_emac_v2_disable_dma_tx(emac_v2_dev);
        qc7xx_emac_v2_take_desc_ownership_tx(emac_v2_dev);
        qc7xx_emac_v2_enable_dma_tx(emac_v2_dev);
        EMAC_V2_DRV_WARN("EMAC_V2 DMA TX Stopped Error\r\n");
    }
    qc7xx_emac_v2_enable_interrupt(emac_v2_dev, EMAC_V2_INT_ENABLE_CFG);
}

int qc7xx_emac_queue_rx_push(struct qc7xx_device_s *dev, struct qc7xx_emac_trans_desc_s *rx_desc)
{
    /* lock */
    uintptr_t flag = qc7xx_irq_save();

    qc7xx_emac_v2_set_rx_qptr(dev, (uint32_t)rx_desc->buff_addr, ETH_RX_BUFFER_SIZE, 0, 0, 0, 0);
    /* unlock */
...
int qc7xx_emac_queue_tx_push(struct qc7xx_device_s *dev, struct qc7xx_emac_trans_desc_s *tx_desc)
{
    /* lock */
    uintptr_t flag = qc7xx_irq_save();

    int ret = qc7xx_emac_v2_bd_tx_enqueue(dev, EMAC_V2_NOCOPY_PACKET, tx_desc->data_len, tx_desc->buff_addr);
...
int qc7xx_emac_bd_ctrl_clean(struct qc7xx_device_s *dev)
{
    return 0;
}
...
#if defined(QCC75X)
/**
 * @brief qc7xx emac phy register read
 *
 * @param dev
 * @param phy_reg
 * @param phy_reg_val
 ...
 *
 */
int qc7xx_emac_phy_reg_read(struct qc7xx_device_s *dev, uint16_t phy_reg, uint16_t *phy_reg_val)
{
    return qc7xx_emac_v2_phy_reg_read(dev, phy_reg, phy_reg_val);
}
/**
 * @brief qc7xx emac phy register write
 *
 * @param dev
 * @param phy_reg
 * @param phy_reg_val
 ...
 *
 */
int qc7xx_emac_phy_reg_write(struct qc7xx_device_s *dev, uint16_t phy_reg, uint16_t phy_reg_val)
{
    return qc7xx_emac_v2_phy_reg_write(dev, phy_reg, phy_reg_val);
}

/**
 * @brief qc7xx emac feature control
 *
 * @param dev
 * @param cmd
 * @param arg
...
int qc7xx_emac_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
    return qc7xx_emac_v2_feature_control(dev, cmd, arg);
}
#endif
#endif