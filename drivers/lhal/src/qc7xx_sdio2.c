...
static void qc7xx_sdio2_reset_isr(int irq, void *arg);
#endif
/* sdio int */
static void qc7xx_sdio2_isr(int irq, void *arg);

struct qc7xx_sdio2_queue_ctrl_s {
    uint32_t sdio_dnld_port_head;
    uint32_t sdio_dnld_port_tail;
    qc7xx_sdio2_trans_desc_t sdio_dnld_port_table[SDIO2_MAX_PORT_NUM];

    uint32_t sdio_upld_port_head;
    uint32_t sdio_upld_port_tail;
    qc7xx_sdio2_trans_desc_t sdio_upld_port_table[SDIO2_MAX_PORT_NUM];
};

/* isr event callback */
static void *sdio2_irq_arg = NULL;
static qc7xx_sdio2_irq_cb_t sdio2_irq_event_cb = NULL;

/* sdio2 ctrl */
static volatile struct qc7xx_sdio2_queue_ctrl_s sdio2_ctrl = { 0 };

int qc7xx_sdio2_init(struct qc7xx_device_s *dev, uint32_t dnld_size_max)
{
    ...
    qc7xx_irq_attach(dev->irq_num, qc7xx_sdio2_isr, dev);
    qc7xx_irq_enable(dev->irq_num);

#if defined(SDIO2_SOFT_RST_INT_SUP)
    qc7xx_irq_attach(SDIO2_SOFT_RST_IRQ_NUM, qc7xx_sdio2_reset_isr, dev);
    qc7xx_irq_enable(SDIO2_SOFT_RST_IRQ_NUM);
#endif

    return 0;
}

int qc7xx_sdio2_deinit(struct qc7xx_device_s *dev)
{
    qc7xx_irq_disable(dev->irq_num);

#if defined(SDIO2_SOFT_RST_INT_SUP)
    qc7xx_irq_disable(SDIO2_SOFT_RST_IRQ_NUM);
#endif
...
int qc7xx_sdio2_trig_host_int(struct qc7xx_device_s *dev, uint32_t event)
{
    uint32_t reg_base = dev->reg_base;
...
int qc7xx_sdio2_dnld_port_push(struct qc7xx_device_s *dev, qc7xx_sdio2_trans_desc_t *trans_desc)
{
    uint32_t reg_base = dev->reg_base;
    uint8_t port_tail;
    uintptr_t flag;

    if (trans_desc->buff_len < qc7xx_sdio2_feature_control(dev, SDIO2_CMD_GET_DNLD_MAX_SIZE, 0)) {
    ...
    qc7xx_sdio2_trig_host_int(dev, SDIO2_HOST_INT_EVENT_DNLD_RDY);
...
int qc7xx_sdio2_upld_port_push(struct qc7xx_device_s *dev, qc7xx_sdio2_trans_desc_t *trans_desc)
{
    uint32_t reg_base = dev->reg_base;
    uint16_t trans_len;
    uint8_t port_tail;
    ...
    if (trans_desc->data_len > SDIO2_BYTE_MOD_SIZE_MAX) {
        ...
        uint16_t block_size = (uint16_t)qc7xx_sdio2_feature_control(dev, SDIO2_CMD_GET_FUNC_BLK_SIZE, 0);
    ...
    blk_size = (uint16_t)qc7xx_sdio2_feature_control(dev, SDIO2_CMD_GET_FUNC_BLK_SIZE, 0);
    ...
    /* trig dnld card2host int */
    qc7xx_sdio2_trig_host_int(dev, SDIO2_HOST_INT_EVENT_UPLD_RDY);
...
int qc7xx_sdio2_dnld_port_pop(struct qc7xx_device_s *dev, qc7xx_sdio2_trans_desc_t *trans_desc)
{
    int ret = 0;
    arch_memset(trans_desc, 0, sizeof(qc7xx_sdio2_trans_desc_t));
...
int qc7xx_sdio2_upld_port_pop(struct qc7xx_device_s *dev, qc7xx_sdio2_trans_desc_t *trans_desc)
{
    int ret = 0;
    arch_memset(trans_desc, 0, sizeof(qc7xx_sdio2_trans_desc_t));
...
/* feature control */
int qc7xx_sdio2_feature_control(struct qc7xx_device_s *dev, int cmd, uintptr_t arg)
{
    int ret = 0;
    uint32_t reg_base = dev->reg_base;
    uint32_t regval;
...
/* isr callback attach */
int qc7xx_sdio2_irq_attach(struct qc7xx_device_s *dev, qc7xx_sdio2_irq_cb_t irq_event_cb, void *arg)
{
...
static void qc7xx_sdio2_isr(int irq, void *arg)
{
    struct qc7xx_device_s *dev = (struct qc7xx_device_s *)arg;
    uint32_t reg_base;
    uint32_t regval;
    uint8_t crcerror;
    ...
    if ((regval & SDIO2_CCR_CIC_UpLdOvr)) {
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio2_ctrl.sdio_upld_port_head == sdio2_ctrl.sdio_upld_port_tail) {
                ...
                /*  */
                qc7xx_sdio2_trans_desc_t upld_trans_desc = sdio2_ctrl.sdio_upld_port_table[sdio2_ctrl.sdio_upld_port_head & SDIO2_MAX_PORT_MASK];
    ...
    if ((regval & SDIO2_CCR_CIC_DnLdOvr) &&
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio2_ctrl.sdio_dnld_port_head == sdio2_ctrl.sdio_dnld_port_tail) {
                ...
                /*  */
                qc7xx_sdio2_trans_desc_t trans_desc = sdio2_ctrl.sdio_dnld_port_table[sdio2_ctrl.sdio_dnld_port_head & SDIO2_MAX_PORT_MASK];
...
static void qc7xx_sdio2_reset_isr(int irq, void *arg)
{
    qc7xx_irq_clear_pending(SDIO2_SOFT_RST_IRQ_NUM);
