...
#include "qc7xx_uart.h"
#include "qc7xx_clock.h"
#include "hardware/uart_reg.h"

#define UART_TX_TIMEOUT_COUNT (160 * 1000)

void qc7xx_uart_init(struct qc7xx_device_s *dev, const struct qc7xx_uart_config_s *config)
{
...
#ifdef romapi_qc7xx_uart_init
    romapi_qc7xx_uart_init(dev, config);
#else
    uint32_t div = 0;
    uint32_t tx_cfg;
    uint32_t rx_cfg;
    ...
    div = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_UART, dev->idx) * 10 / config->baudrate + 5) / 10;
...
void qc7xx_uart_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_deinit
    romapi_qc7xx_uart_deinit(dev);
#else
    uint32_t reg_base;
    uint32_t tx_cfg;
    uint32_t rx_cfg;
...
void qc7xx_uart_enable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_enable
    romapi_qc7xx_uart_enable(dev);
#else
    uint32_t reg_base;
    uint32_t tx_cfg;
    uint32_t rx_cfg;
...
void qc7xx_uart_disable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_disable
    romapi_qc7xx_uart_disable(dev);
#else
    uint32_t reg_base;
    uint32_t tx_cfg;
    uint32_t rx_cfg;
...
void qc7xx_uart_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_uart_link_txdma
    romapi_qc7xx_uart_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_uart_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_uart_link_rxdma
    romapi_qc7xx_uart_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
ATTR_TCM_SECTION int qc7xx_uart_putchar(struct qc7xx_device_s *dev, int ch)
{
#ifdef romapi_qc7xx_uart_putchar
    return romapi_qc7xx_uart_putchar(dev, ch);
#else
    uint64_t start_time;
    uint32_t reg_base;
...
ATTR_TCM_SECTION int qc7xx_uart_getchar(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_getchar
    return romapi_qc7xx_uart_getchar(dev);
#else
    int ch = -1;
    uint32_t reg_base;
...
ATTR_TCM_SECTION int qc7xx_uart_put(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
#ifdef romapi_qc7xx_uart_put
    return romapi_qc7xx_uart_put(dev, data, len);
#else
    int ret;
    for (uint32_t i = 0; i < len; i++) {
        ret = qc7xx_uart_putchar(dev, data[i]);
        if (ret < 0) {
            return ret;
        }
    }
...
ATTR_TCM_SECTION int qc7xx_uart_put_block(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
#ifdef romapi_qc7xx_uart_put_block
    return romapi_qc7xx_uart_put_block(dev, data, len);
#else
    int ret;
...
ATTR_TCM_SECTION int qc7xx_uart_get(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
#ifdef romapi_qc7xx_uart_get
    return romapi_qc7xx_uart_get(dev, data, len);
#else
    int ch = -1;
    uint32_t count = 0;

    while (count < len) {
        if ((ch = qc7xx_uart_getchar(dev)) < 0) {
            break;
        }
        data[count] = ch;
        count++;
...
int qc7xx_uart_wait_tx_done(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_wait_tx_done
    return romapi_qc7xx_uart_wait_tx_done(dev);
#else
    uint64_t start_time;

    start_time = qc7xx_mtimer_get_time_ms();
    while (!qc7xx_uart_txempty(dev)) {
        if ((qc7xx_mtimer_get_time_ms() - start_time) > 1000) {
            return -ETIMEDOUT;
        }
    }
...
bool qc7xx_uart_txready(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_txready
    return romapi_qc7xx_uart_txready(dev);
#else
    uint32_t reg_base;
...
bool qc7xx_uart_txempty(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_txempty
    return romapi_qc7xx_uart_txempty(dev);
#else
    uint32_t reg_base;
...
bool qc7xx_uart_rxavailable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_rxavailable
    return romapi_qc7xx_uart_rxavailable(dev);
#else
    uint32_t reg_base;
...
void qc7xx_uart_txint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_uart_txint_mask
    romapi_qc7xx_uart_txint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t int_mask;
...
void qc7xx_uart_rxint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_uart_rxint_mask
    romapi_qc7xx_uart_rxint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t int_mask;
...
void qc7xx_uart_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_uart_errint_mask
    romapi_qc7xx_uart_errint_mask(dev, mask);
#else
    uint32_t reg_base;
    uint32_t int_mask;
...
uint32_t qc7xx_uart_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_uart_get_intstatus
    return romapi_qc7xx_uart_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t int_status;
    uint32_t int_mask;
...
void qc7xx_uart_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
#ifdef romapi_qc7xx_uart_int_clear
    romapi_qc7xx_uart_int_clear(dev, int_clear);
#else
    uint32_t reg_base;
...
{
#ifdef romapi_qc7xx_uart_feature_control
    return romapi_qc7xx_uart_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t tmp;
    ...
    switch (cmd) {
        case UART_CMD_SET_BAUD_RATE:
            qc7xx_uart_disable(dev);

            /* Cal the baud rate divisor */
            tmp = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_UART, dev->idx) * 10 / arg + 5) / 10;

            putreg32(((tmp - 1) << 0x10) | ((tmp - 1) & 0xFFFF), reg_base + UART_BIT_PRD_OFFSET);
            qc7xx_uart_enable(dev);
            break;
        ...
        case UART_CMD_IR_CONFIG: {
            struct qc7xx_uart_ir_config_s *ir_config = (struct qc7xx_uart_ir_config_s *)arg;
            tx_tmp = getreg32(reg_base + UART_UTX_CONFIG_OFFSET);
            if (ir_config->tx_en) {
