...
void qc7xx_i2s_init(struct qc7xx_device_s *dev, const struct qc7xx_i2s_config_s *config)
{
...
#ifdef romapi_qc7xx_i2s_init
    romapi_qc7xx_i2s_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t div;
    ...
    div = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_I2S, dev->idx) / 2 * 10 / config->bclk_freq_hz + 5) / 10;
    div = (div) ? (div - 1) : 0;
...
void qc7xx_i2s_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_i2s_deinit
    romapi_qc7xx_i2s_deinit(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_i2s_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_i2s_link_txdma
    romapi_qc7xx_i2s_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_i2s_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_i2s_link_rxdma
    romapi_qc7xx_i2s_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_i2s_txint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_i2s_txint_mask
    romapi_qc7xx_i2s_txint_mask(dev, mask);
#else
    uint32_t regval;
...
void qc7xx_i2s_rxint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_i2s_rxint_mask
    romapi_qc7xx_i2s_rxint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
void qc7xx_i2s_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_i2s_errint_mask
    romapi_qc7xx_i2s_errint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
uint32_t qc7xx_i2s_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_i2s_get_intstatus
    return romapi_qc7xx_i2s_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t int_status;
    uint32_t int_mask;
...
int qc7xx_i2s_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_i2s_feature_control
    return romapi_qc7xx_i2s_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
    ...
    switch (cmd) {
        ...
        case I2S_CMD_GET_TX_FIFO_CNT:
            /* Get TX_FIFO available count */
            regval = getreg32(reg_base + I2S_FIFO_CONFIG_1_OFFSET);
            ret = (regval & I2S_TX_FIFO_CNT_MASK) >> I2S_TX_FIFO_CNT_SHIFT;
            break;

        case I2S_CMD_GET_RX_FIFO_CNT:
            /* clear rx fifo */
            regval = getreg32(reg_base + I2S_FIFO_CONFIG_1_OFFSET);
            ret = (regval & I2S_RX_FIFO_CNT_MASK) >> I2S_RX_FIFO_CNT_SHIFT;
            break;
        ...
        case I2S_CMD_MONO_CHANEL_SEL:
            /* Mono channel select, arg use true or false, false: L-channel, true: R-channel */
            regval = getreg32(reg_base + I2S_CONFIG_OFFSET);
            if (arg) {
                /* rx mono mode R-channel */
                regval |= I2S_CR_MONO_RX_CH;
            } else {
                /* rx mono mode L-channel */
                regval &= ~I2S_CR_MONO_RX_CH;
