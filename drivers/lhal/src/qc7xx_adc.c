...
void qc7xx_adc_init(struct qc7xx_device_s *dev, const struct qc7xx_adc_config_s *config)
{
    ...
    */
    ...
    if (adc_cali_complete == 0) {
        coe = qc7xx_efuse_get_adc_trim(); /* read from efuse */
        qc7xx_update_adc_trim(dev, config);
        tsen_offset = qc7xx_efuse_get_adc_tsen_trim(); /* read from efuse */
        adc_cali_complete = 1;
    }
}

void qc7xx_update_adc_trim(struct qc7xx_device_s *dev, const struct qc7xx_adc_config_s *config)
{
    uint32_t regval;
    uint32_t reg_base;
    uint32_t os_val = 0;
...
void qc7xx_adc_set_reference_channel(int channel, int32_t millivolt)
{
    adc_reference_channel = channel;
    adc_reference_channel_millivolt = millivolt;
}

int qc7xx_adc_channel_config(struct qc7xx_device_s *dev, struct qc7xx_adc_channel_s *chan, uint8_t channels)
{
    uint32_t regval;
    uint32_t regval2;
    uint32_t reg_base;
...
void qc7xx_adc_rxint_mask(struct qc7xx_device_s *dev, bool mask)
{
    uint32_t regval;
...
void qc7xx_adc_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
    uint32_t regval;
    uint32_t regval2;
    uint32_t reg_base;
...
uint32_t qc7xx_adc_get_intstatus(struct qc7xx_device_s *dev)
{
    uint32_t reg_base;
    uint32_t regval = 0;
...
void qc7xx_adc_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
    uint32_t regval;
    uint32_t reg_base;
...
static uint32_t qc7xx_get_conv_value(uint32_t os1, int os2, uint32_t val)
{
    int conv_val = 0;
...
void qc7xx_adc_parse_result(struct qc7xx_device_s *dev, uint32_t *buffer, struct qc7xx_adc_result_s *result, uint16_t count)
{
    ...
    if (diff_mode == 0) {
        ...
        for (uint16_t i = 0; i < count; i++) {
            ...
            conv_result = (uint32_t)((buffer[i] & 0xffff));
            result[i].value = qc7xx_get_conv_value(os1, os2, conv_result);
            if (resolution == ADC_RESOLUTION_12B) {
                result[i].value >>= 4;
...
void qc7xx_adc_vbat_enable(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_adc_vbat_disable(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
