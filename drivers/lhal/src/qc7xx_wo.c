...
void qc7xx_wo_pin_init(struct qc7xx_device_s *dev, uint8_t pin, uint8_t mode)
{
#ifdef romapi_qc7xx_wo_pin_init
    romapi_qc7xx_wo_pin_init(dev, pin, mode);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_init(struct qc7xx_device_s *dev, struct qc7xx_wo_cfg_s *cfg)
{
#ifdef romapi_qc7xx_wo_init
    romapi_qc7xx_wo_init(dev, cfg);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_enable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_enable
    romapi_qc7xx_wo_enable(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_disable(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_disable
    romapi_qc7xx_wo_disable(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
uint32_t qc7xx_wo_get_fifo_available_cnt(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_get_fifo_available_cnt
    return romapi_qc7xx_wo_get_fifo_available_cnt(dev);
#else
    uint32_t regval;
...
uint32_t qc7xx_wo_push_fifo(struct qc7xx_device_s *dev, uint16_t *data, uint32_t len)
{
#ifdef romapi_qc7xx_wo_push_fifo
    return romapi_qc7xx_wo_push_fifo(dev, data, len);
#else
    uint32_t idx = 0;
    uint32_t fclk = qc7xx_clk_get_system_clock(QC7XX_SYSTEM_CPU_CLK);
    uint32_t xclk = qc7xx_clk_get_system_clock(QC7XX_SYSTEM_XCLK);
    uint32_t to_max = fclk / xclk * 512;
    uint32_t to = to_max;

    while (idx < len) {
        if (qc7xx_wo_get_fifo_available_cnt(dev) > 0) {
...
void qc7xx_wo_clear_fifo(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_clear_fifo
    romapi_qc7xx_wo_clear_fifo(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_enable_dma(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_enable_dma
    romapi_qc7xx_wo_enable_dma(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_disable_dma(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_disable_dma
    romapi_qc7xx_wo_disable_dma(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
uint32_t qc7xx_wo_get_int_status(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_wo_get_int_status
    return romapi_qc7xx_wo_get_int_status(dev);
#else
    uint32_t regval;
    uint32_t sts = 0;
...
void qc7xx_wo_int_mask(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_wo_int_mask
    romapi_qc7xx_wo_int_mask(dev, int_type);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_int_unmask(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_wo_int_unmask
    romapi_qc7xx_wo_int_unmask(dev, int_type);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_int_clear(struct qc7xx_device_s *dev, uint32_t int_type)
{
#ifdef romapi_qc7xx_wo_int_clear
    romapi_qc7xx_wo_int_clear(dev, int_type);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_wo_uart_init(struct qc7xx_device_s *dev, uint32_t baudrate, uint8_t pin)
{
    struct qc7xx_wo_cfg_s cfg;

    if (baudrate == 0) {
        baudrate = 2 * 1000 * 1000;
    }
    baudrate = qc7xx_clk_get_system_clock(QC7XX_SYSTEM_XCLK) / baudrate;
    ...
    qc7xx_wo_pin_init(dev, pin, WO_MODE_SET_CLR);
    qc7xx_wo_init(dev, &cfg);
    wo_uart_pin = pin;
...
void qc7xx_wo_uart_putchar(struct qc7xx_device_s *dev, uint8_t ch)
{
    ...
    qc7xx_wo_push_fifo(dev, wo_uart_buff16, 10);
}

void qc7xx_wo_uart_put(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
    for (uint32_t i = 0; i < len; i++) {
        qc7xx_wo_uart_putchar(dev, data[i]);
    }
}
