...
__UNUSED static void qc7xx_i2c_addr_config(struct qc7xx_device_s *dev, uint16_t slaveaddr, uint8_t *subaddr, uint8_t subaddr_size, bool is_addr_10bit)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline void qc7xx_i2c_set_dir(struct qc7xx_device_s *dev, bool is_in)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline void qc7xx_i2c_set_datalen(struct qc7xx_device_s *dev, uint16_t data_len)
{
    uint32_t regval;
    uint32_t reg_base;
...
__UNUSED static void qc7xx_i2c_set_frequence(struct qc7xx_device_s *dev, uint32_t freq)
{
    uint32_t regval;
    uint32_t reg_base;
    ...
    phase = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_I2C, dev->idx) + freq / 2) / freq;
    if (freq <= 100 * 1000) {
...
static inline bool qc7xx_i2c_isbusy(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline bool qc7xx_i2c_isend(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline bool qc7xx_i2c_isnak(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline void qc7xx_i2c_enable(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline void qc7xx_i2c_disable(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
static inline bool qc7xx_i2c_isenable(struct qc7xx_device_s *dev)
{
    uint32_t regval;
    uint32_t reg_base;
...
__UNUSED static int qc7xx_i2c_write_bytes(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
    uint32_t reg_base;
    uint32_t temp = 0;
    uint8_t *tmp_buf;
    ...
    while (len >= 4) {
        ...
        if (!qc7xx_i2c_isenable(dev)) {
            qc7xx_i2c_enable(dev);
        }
        temp = 0;
    }
    ...
    start_time = qc7xx_mtimer_get_time_ms();
    while (qc7xx_i2c_isbusy(dev) || !qc7xx_i2c_isend(dev) || qc7xx_i2c_isnak(dev)) {
        if ((qc7xx_mtimer_get_time_ms() - start_time) > 100) {
            return -ETIMEDOUT;
        }
    }
    qc7xx_i2c_disable(dev);

    return 0;
}

__UNUSED static int qc7xx_i2c_read_bytes(struct qc7xx_device_s *dev, uint8_t *data, uint32_t len)
{
    uint32_t reg_base;
    uint32_t temp = 0;
    uint8_t *tmp_buf;
    ...
    start_time = qc7xx_mtimer_get_time_ms();
    while (qc7xx_i2c_isbusy(dev) || !qc7xx_i2c_isend(dev)) {
        if ((qc7xx_mtimer_get_time_ms() - start_time) > 100) {
            return -ETIMEDOUT;
        }
    }
...
void qc7xx_i2c_init(struct qc7xx_device_s *dev, uint32_t frequency)
{
#ifdef romapi_qc7xx_i2c_init
    romapi_qc7xx_i2c_init(dev, frequency);
#else
    uint32_t regval;
    uint32_t reg_base;
    ...
    qc7xx_i2c_set_frequence(dev, frequency);
#endif
}

void qc7xx_i2c_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_i2c_deinit
    romapi_qc7xx_i2c_deinit(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qc7xx_i2c_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_i2c_link_txdma
    romapi_qc7xx_i2c_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_i2c_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_i2c_link_rxdma
    romapi_qc7xx_i2c_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_i2c_transfer(struct qc7xx_device_s *dev, struct qc7xx_i2c_msg_s *msgs, int count)
{
#ifdef romapi_qc7xx_i2c_transfer
    return romapi_qc7xx_i2c_transfer(dev, msgs, count);
#else
    ...
    for (uint16_t i = 0; i < count; i++) {
        ...
        if (msgs[i].flags & I2C_M_NOSTOP) {
            ...
            qc7xx_i2c_addr_config(dev, msgs[i].addr, msgs[i].buffer, subaddr_size, is_addr_10bit);
            i++;
        } else {
            subaddr_size = 0;
            qc7xx_i2c_addr_config(dev, msgs[i].addr, msgs[i].buffer, subaddr_size, is_addr_10bit);
        ...
        qc7xx_i2c_set_datalen(dev, msgs[i].length);
        if (msgs[i].flags & I2C_M_READ) {
            qc7xx_i2c_set_dir(dev, 1);
            if ((msgs[i].flags & I2C_M_DMA) == 0) {
                ret = qc7xx_i2c_read_bytes(dev, msgs[i].buffer, msgs[i].length);
                if (ret < 0) {
                    return ret;
                }
            } else {
            ...
            qc7xx_i2c_set_dir(dev, 0);
            if ((msgs[i].flags & I2C_M_DMA) == 0) {
                ret = qc7xx_i2c_write_bytes(dev, msgs[i].buffer, msgs[i].length);
                if (ret < 0) {
                    return ret;
                }
            } else {
...
void qc7xx_i2c_int_mask(struct qc7xx_device_s *dev, uint32_t int_type, bool mask)
{
#ifdef romapi_qc7xx_i2c_int_mask
    romapi_qc7xx_i2c_int_mask(dev, int_type, mask);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_i2c_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
#ifdef romapi_qc7xx_i2c_int_clear
    romapi_qc7xx_i2c_int_clear(dev, int_clear);
#else
    uint32_t reg_base;
    uint32_t regval;
...
uint32_t qc7xx_i2c_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_i2c_get_intstatus
    return romapi_qc7xx_i2c_get_intstatus(dev);
#else
    uint32_t reg_base;
...
int qc7xx_i2c_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_i2c_feature_control
    return romapi_qc7xx_i2c_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
    struct qc7xx_i2c_timing_s *timing;
    ...
    switch (cmd) {
        ...
        case I2C_CMD_SET_TIMING:
            timing = (struct qc7xx_i2c_timing_s *)arg;
