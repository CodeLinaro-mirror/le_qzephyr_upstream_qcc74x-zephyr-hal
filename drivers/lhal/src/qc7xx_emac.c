...
/* emac int */
void qc7xx_emac_isr(int irq, void *arg);

struct qc7xx_emac_queue_ctrl_s {
...
static volatile struct qc7xx_emac_queue_ctrl_s emac_ctrl = { 0 };
...
#define GLB_SWRST_CFG0_OFFSET    (0x20000540)
    ...
    /* emac reset */
    regval = getreg32(GLB_SWRST_CFG0_OFFSET);
    regval |= (1 << 23);
    putreg32(regval, GLB_SWRST_CFG0_OFFSET);
    arch_delay_us(1);
    /* emac reset release */
    regval = getreg32(GLB_SWRST_CFG0_OFFSET);
    regval &= ~(1 << 23);
    ...
    /* enable irq */
    qc7xx_irq_attach(dev->irq_num, qc7xx_emac_isr, dev);
...
int qc7xx_emac_deinit(struct qc7xx_device_s *dev)
{
    uint32_t reg_base;
    uint32_t regval;
    ...
    /* clean ctrl */
    arch_memset((void *)&emac_ctrl, 0, sizeof(struct qc7xx_emac_queue_ctrl_s));
...
int qc7xx_emac_md_read(struct qc7xx_device_s *dev, uint8_t phy_addr, uint16_t reg_addr, uint16_t *data)
{
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_emac_md_write(struct qc7xx_device_s *dev, uint8_t phy_addr, uint16_t reg_addr, uint16_t data)
{
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_emac_queue_tx_push(struct qc7xx_device_s *dev, struct qc7xx_emac_trans_desc_s *tx_desc)
{
    ...
    /* hw tx db address */
    uint32_t tx_db_addr = reg_base + EMAC_DMA_DESC_OFFSET + bd_index * sizeof(struct qc7xx_emac_hw_buff_desc_s);
...
int qc7xx_emac_queue_rx_push(struct qc7xx_device_s *dev, struct qc7xx_emac_trans_desc_s *rx_desc)
{
    ...
    /* hw rx db address */
    uint32_t rx_db_addr = reg_base + EMAC_DMA_DESC_OFFSET + (bd_index + 64) * sizeof(struct qc7xx_emac_hw_buff_desc_s);
...
static void qc7xx_emac_isr_cb_rx(struct qc7xx_device_s *dev)
{
    uint32_t reg_base = dev->reg_base;
    ...
    for (uint32_t i = 0; i < rx_bd_num; i++) {
        ...
        struct qc7xx_emac_hw_buff_desc_s rx_bd = *(struct qc7xx_emac_hw_buff_desc_s *)(reg_base + EMAC_DMA_DESC_OFFSET + (rx_bd_index + 64) * sizeof(struct qc7xx_emac_hw_buff_desc_s));
        /* check bd status */
...
static void qc7xx_emac_isr_cb_tx(struct qc7xx_device_s *dev)
{
    uint32_t reg_base = dev->reg_base;
    ...
    for (uint32_t i = 0; i < tx_bd_num; i++) {
        ...
        struct qc7xx_emac_hw_buff_desc_s tx_bd = *(struct qc7xx_emac_hw_buff_desc_s *)(reg_base + EMAC_DMA_DESC_OFFSET + tx_bd_index * sizeof(struct qc7xx_emac_hw_buff_desc_s));
        /* check bd status */
