...
void qc7xx_spi_init(struct qc7xx_device_s *dev, const struct qc7xx_spi_config_s *config)
{
...
#ifdef romapi_qc7xx_spi_init
    romapi_qc7xx_spi_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t div;
...
#if defined(QCC75X)
    uint32_t mini_spi_mode = getreg32(0x00080024);
    if (config->role == SPI_ROLE_MASTER) {
        mini_spi_mode &= ~(1 << 1); // master mode
    } else {
        mini_spi_mode |= (1 << 1); // slave mode
    }
    putreg32(mini_spi_mode, 0x00080024);
#endif
    ...
    div = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_SPI, dev->idx) / 2 * 10 / config->freq + 5) / 10;
    div = (div) ? (div - 1) : 0;
...
void qc7xx_spi_deinit(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_spi_deinit
    romapi_qc7xx_spi_deinit(dev);
#else
    uint32_t regval;
    /* disable SPI */
...
void qc7xx_spi_link_txdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_spi_link_txdma
    romapi_qc7xx_spi_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
void qc7xx_spi_link_rxdma(struct qc7xx_device_s *dev, bool enable)
{
#ifdef romapi_qc7xx_spi_link_rxdma
    romapi_qc7xx_spi_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
ATTR_TCM_SECTION uint32_t qc7xx_spi_poll_send(struct qc7xx_device_s *dev, uint32_t data)
{
#ifdef romapi_qc7xx_spi_poll_send
    return romapi_qc7xx_spi_poll_send(dev, data);
#else
...
/* read and write data */
ATTR_TCM_SECTION int qc7xx_spi_poll_exchange(struct qc7xx_device_s *dev, const void *txbuffer, void *rxbuffer, size_t nbytes)
{
#ifdef romapi_qc7xx_spi_poll_exchange
    return romapi_qc7xx_spi_poll_exchange(dev, txbuffer, rxbuffer, nbytes);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
    uint32_t tx_cnt;
    ...
    if (txbuffer != NULL) {
        ...
        /* Send idle Data */
        for (; fifo_cnt > 0; fifo_cnt--) {
            putreg32(QC7XX_SPI_IDEL_DATA, reg_base + SPI_FIFO_WDATA_OFFSET);
        }
    }
...
void qc7xx_spi_txint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_spi_txint_mask
    romapi_qc7xx_spi_txint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
void qc7xx_spi_rxint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_spi_rxint_mask
    romapi_qc7xx_spi_rxint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
void qc7xx_spi_tcint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_spi_tcint_mask
    romapi_qc7xx_spi_tcint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
void qc7xx_spi_errint_mask(struct qc7xx_device_s *dev, bool mask)
{
#ifdef romapi_qc7xx_spi_errint_mask
    romapi_qc7xx_spi_errint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
uint32_t qc7xx_spi_get_intstatus(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_spi_get_intstatus
    return romapi_qc7xx_spi_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t int_status;
    uint32_t int_mask;
...
void qc7xx_spi_int_clear(struct qc7xx_device_s *dev, uint32_t int_clear)
{
#ifdef romapi_qc7xx_spi_int_clear
    romapi_qc7xx_spi_int_clear(dev, int_clear);
#else
    uint32_t reg_base;
    uint32_t regval;
...
bool qc7xx_spi_isbusy(struct qc7xx_device_s *dev)
{
#ifdef romapi_qc7xx_spi_isbusy
    return romapi_qc7xx_spi_isbusy(dev);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qc7xx_spi_feature_control(struct qc7xx_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qc7xx_spi_feature_control
    return romapi_qc7xx_spi_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
    ...
    switch (cmd) {
        ...
        case SPI_CMD_SET_FREQ:
            ...
            /* integer frequency segmentation by rounding */
            div = (qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_SPI, dev->idx) / 2 * 10 / arg + 5) / 10;
            div = (div) ? (div - 1) : 0;
            div = (div > 0xff) ? 0xff : div;
        ...
        case SPI_CMD_GET_FREQ:
            ...
            ret = qc7xx_clk_get_peripheral_clock(QC7XX_DEVICE_TYPE_SPI, dev->idx) / div;
            break;
