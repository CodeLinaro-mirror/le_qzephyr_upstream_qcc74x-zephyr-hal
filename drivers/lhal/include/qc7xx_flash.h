...
#ifndef _QC7XX_FLASH_H
#define _QC7XX_FLASH_H
...
struct qc7xx_flash_aes_config_s {
    uint8_t region;
 ...
 */
void qc7xx_flash_get_cfg(uint8_t **cfg_addr, uint32_t *len);
 ...
 */
void qc7xx_flash_set_iomode(uint8_t iomode);
 ...
 */
uint32_t qc7xx_flash_get_image_offset(void);
 ...
 * @return Zero on success; a negated errno value on failure
 */
int qc7xx_flash_erase(uint32_t addr, uint32_t len);
 ...
 * @return Zero on success; a negated errno value on failure
 */
int qc7xx_flash_write(uint32_t addr, uint8_t *data, uint32_t len);
 ...
 * @return Zero on success; a negated errno value on failure
 */
int qc7xx_flash_read(uint32_t addr, uint8_t *data, uint32_t len);
 ...
 * @return int
 */
int qc7xx_flash_get_unique_id(uint8_t *data, uint8_t id_len);
 ...
 * @return Zero on success; a negated errno value on failure
 */
int qc7xx_flash_set_cache(uint8_t cont_read, uint8_t cache_enable, uint8_t cache_way_disable, uint32_t flash_offset);
 ...
 */
void qc7xx_flash_aes_init(struct qc7xx_flash_aes_config_s *config);

/**
 * @brief Enable flash decrypt with aes.
 *
 */
void qc7xx_flash_aes_enable(void);

/**
 * @brief Disable flash decrypt with aes.
 *
 */
void qc7xx_flash_aes_disable(void);
 ...
 */
void qc7xx_flash_jump_encrypted_app(uint8_t index, uint32_t flash_addr, uint32_t len);
