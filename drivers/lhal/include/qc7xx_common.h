...
#ifndef _QC7XX_COMMON_H
#define _QC7XX_COMMON_H
 ...
 * @return [in] uint32_t
 */
uint32_t  qc7xx_soft_crc32_ex(uint32_t initial, void *in, uint32_t len);
 ...
 * @return [in] uint32_t
 */
uint32_t qc7xx_soft_crc32(void *in, uint32_t len);
...
#else
#define qc7xx_check_cache_addr_aligned(addr)  (true)
#endif
...
int32_t qc7xx_set_boot2_version_to_efuse(uint8_t version);
int32_t qc7xx_get_boot2_info_from_flash(qc7xx_verinf_t *version);

static inline __attribute__((always_inline)) void qc7xx_lhal_assert_func(const char *file, uint32_t line, const char *function, const char *string)
{
