...
void twt_setup_req_set_conf_wake_int_mantissa(void *pa, uint16_t val) {
    ...
    struct_func_get_def(twt_setup_ind, uint8_t, resp_type);
    struct_func_get_def(twt_setup_ind, uint8_t, sta_idx);
    struct_func_get_def(twt_setup_ind, uint8_t, vif_idx);
    struct_func_get_def(twt_setup_ind, uint8_t, flow_id);
    struct_func_get_def(twt_setup_ind, uint8_t, trigger);
    struct_func_get_def(twt_setup_ind, uint8_t, implicit);
    struct_func_get_def(twt_setup_ind, uint8_t, dialog_token);
uint8_t twt_setup_ind_get_conf_flow_type(void *pa) {
    struct twt_setup_ind *p = (struct twt_setup_ind *)pa;
    return p->conf.flow_type;
};
uint8_t twt_setup_ind_get_conf_wake_int_exp(void *pa) {
    struct twt_setup_ind *p = (struct twt_setup_ind *)pa;
    return p->conf.wake_int_exp;
};
bool twt_setup_ind_get_conf_wake_dur_unit(void *pa) {
    struct twt_setup_ind *p = (struct twt_setup_ind *)pa;
    return p->conf.wake_dur_unit;
};
uint8_t twt_setup_ind_get_conf_min_twt_wake_dur(void *pa) {
    struct twt_setup_ind *p = (struct twt_setup_ind *)pa;
    return p->conf.min_twt_wake_dur;
};
uint16_t twt_setup_ind_get_conf_wake_int_mantissa(void *pa) {
    struct twt_setup_ind *p = (struct twt_setup_ind *)pa;
    return p->conf.wake_int_mantissa;
};
    ...
    void twt_statusget_cfm_get_conf(void *pa, void *cfm_ptr,
                                    uint8_t flow_conf_max)
{
    struct twt_status_info *p = (struct twt_status_info *)pa;
    struct twt_statusget_cfm *cfm = (struct twt_statusget_cfm *)cfm_ptr;
    uint8_t max = (flow_conf_max > NX_TWT_FLOW_NB) ? NX_TWT_FLOW_NB :
                  flow_conf_max;

    memcpy(p, cfm->conf, sizeof(struct twt_status_info) * max);
}
