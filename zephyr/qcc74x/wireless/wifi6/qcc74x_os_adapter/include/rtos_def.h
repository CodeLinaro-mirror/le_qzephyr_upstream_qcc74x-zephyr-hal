...
#ifdef CFG_QCC74x_WIFI_PS_ENABLE
typedef struct rtos_zephyr_timer *rtos_timer_t;
...
enum
{
    ...
    /// RX task stack size
    FHOST_RX_STACK_SIZE = (768+256),
    /// TX task stack size
    FHOST_TX_STACK_SIZE = (384+256),
    /// WPA task stack size
    FHOST_WPA_STACK_SIZE = (WPA_SUPPLICANT_STACK_SIZE+2048),
    /// IPC task stack size
    FHOST_IPC_STACK_SIZE = (512+256),
    /// TG send stack size
    FHOST_TG_SEND_STACK_SIZE = 1024,// zephyr not used
    /// Ping send stack size
    FHOST_PING_SEND_STACK_SIZE = 256,// zephyr not used
    /// Smartconfig task stack size
    FHOST_SMARTCONF_STACK_SIZE = 512,// zephyr not used
    /// IPERF task stack size
    FHOST_IPERF_STACK_SIZE = 1024,// zephyr not used
    /// DOORBELL task stack size
    FHOST_DOORBELL_STACK_SIZE = 1024,// zephyr not used
};

#define RTOS_portTASK_FUNCTION(vFunction, pvParameters, CURRENT_TASK_HANDLE, pvParameters3) void vFunction(void* pvParameters, void * CURRENT_TASK_HANDLE, void * pvParameters3)
#define RTOS_TASK_FCT(name)        RTOS_portTASK_FUNCTION(name, env, CURRENT_TASK_HANDLE, p3)
