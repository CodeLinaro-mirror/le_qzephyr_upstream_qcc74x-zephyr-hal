...
{
    return qcc74x_os_malloc(size);
}

void platform_free(void* mem_ptr)
{
    qcc74x_os_free(mem_ptr);
}
...
{
    return qcc74x_efuse_read_mac(mac);
}

int platform_get_random(unsigned char *buf, size_t len)
{
    return qcc74x_rand_stream(buf, len);
}
...
{
    aos_post_event((uint16_t)catalogue, (uint16_t)code, 0);
}
...
void platform_hook_beacon(uint32_t rhd, uint32_t tim, bcn_param_t* param)
{
    ...
    /* get rtc timestamp */
    rtc_stamp_us = QCC74x_PDS_CNT_TO_US(rtc_cnt);
    ...
    qc7xx_lp_beacon_interval_update(bcn->bcnint);
    if (tim != 0U) {
        /* mm_compute_beacon_crc() reports TIM in Wi-Fi HW address space. */
...
{
    if (prevent) {
        qcc74x_pm_event_bit_set(event);
    } else {
        qcc74x_pm_event_bit_clear(event);
    }
}
#endif
