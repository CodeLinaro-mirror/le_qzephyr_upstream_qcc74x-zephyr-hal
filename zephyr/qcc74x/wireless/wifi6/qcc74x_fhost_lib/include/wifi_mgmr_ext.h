...
#define  CODE_WIFI_ON_TWT_SETUP         26
...
#ifndef QCC74X_TWT_STATUS_INFO_DEFINED
#define QCC74X_TWT_STATUS_INFO_DEFINED
...
/**
 * wifi_mgmr_sta_rssi_update_from_lpfw
 * Update STA RSSI value from low power firmware (LPFW)
 * param:
 ...
 *  -1 : Failed
 */
int wifi_mgmr_sta_rssi_update_from_lpfw(int rssi);
...
#if (defined(NX_COEX_POWERSAVE) && NX_COEX_POWERSAVE) || defined(CONFIG_QCC74X_WIFI_COEX)
int wifi_mgmr_sta_coex_enable(void);
int wifi_mgmr_sta_coex_disable(void);
int wifi_mgmr_sta_coex_duty_set(uint8_t active_ms);
int wifi_mgmr_sta_coex_duty_get(void);
int wifi_mgmr_sta_coex_status_get(void);
#endif
 ...
 * Get current negotiated STA TWT flows.
 * return: 0 on success, negative on failure
 */
int wifi_mgmr_sta_twt_statusget(struct twt_status_info *conf, uint8_t *twt_num);
void wifi_mgmr_sta_twt_cache_clear(void);
void wifi_mgmr_sta_twt_setup_ind_save(const struct twt_setup_ind_info *info);
int wifi_mgmr_sta_twt_setup_ind_get(struct twt_setup_ind_info *info);

/**
 * wifi_mgmr_twt_support
 * Get AP TWT responder support for the current STA connection.
 * return: 1 if supported, 0 if unsupported, negative on failure
 */
int wifi_mgmr_twt_support(void);

/**
 * wifi_mgmr_sta_twt_flow_get
 * Get negotiated STA TWT flow count.
 * return: negotiated TWT flow count
 */
