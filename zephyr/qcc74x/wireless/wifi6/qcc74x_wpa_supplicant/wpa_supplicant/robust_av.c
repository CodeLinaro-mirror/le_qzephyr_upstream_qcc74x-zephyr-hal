			   ...
			   const struct ipv4_params *v4)
{
	...
	if (v4->param_mask & BIT(1)) {
		if (!zsock_inet_ntop(AF_INET, &v4->src_ip, addr,
				     INET_ADDRSTRLEN)) {
			wpa_printf(MSG_ERROR,
	...
	if (v4->param_mask & BIT(2)) {
		if (!zsock_inet_ntop(AF_INET, &v4->dst_ip, addr,
				     INET_ADDRSTRLEN)) {
			wpa_printf(MSG_ERROR,
			   ...
			   const struct ipv6_params *v6)
{
	...
	if (v6->param_mask & BIT(1)) {
		if (!zsock_inet_ntop(AF_INET6, &v6->src_ip, addr,
				     INET6_ADDRSTRLEN)) {
			wpa_printf(MSG_ERROR,
	...
	if (v6->param_mask & BIT(2)) {
		if (!zsock_inet_ntop(AF_INET6, &v6->dst_ip, addr,
				     INET6_ADDRSTRLEN)) {
			wpa_printf(MSG_ERROR,
