...
extern uint32_t __lpfw_share_start__[];
extern uint32_t __lpfw_share_end__[];

#ifdef CONFIG_LPAPP
#define TST_LPFW_SHARE_PTR(ptr) (((uint32_t)(ptr)) < (uint32_t)__lpfw_share_start__ || \
                                 ((uint32_t)(ptr)) >= (uint32_t)__lpfw_share_end__)
#else
#define TST_LPFW_SHARE_PTR(ptr) (1)
#endif
...
static int tcpip_src_addr_cmp(struct mac_eth_hdr *hdr, const uint8_t addr[])
{
    ...
    for (i = 0; i < 6; i++) {
        if (((uint8_t *)&hdr->sa.array)[i] != (uint8_t)(addr[i])) {
            return 1;
        }
    }
...
int net_al_tx_req(struct net_al_tx_req req)
{
    ...
    if (buf) {
        ...
        ASSERT_ERR(staid < STA_MAX || staid == INVALID_STA_IDX);
        if (staid == INVALID_STA_IDX) {
...
{
    qcc74x_custom_tx_callback_t cb = arg;
    if (cb) {
...
#if QCC74x_ENABLE_PACKET_SOCKET
    struct netconn *conn;
    struct netbuf *buf;
#endif
    int i;
